...
{
	__raw_writel(0x4010fe06, EXYNOS5433_WAKEUP_MASK);
	__raw_writel(0x00000000, EXYNOS5433_WAKEUP_MASK1);
				...
				int index)
{
	...
	aftr_wakeup_stat[aftr_wakeup_count].wakeup_stat = __raw_readl(EXYNOS5433_WAKEUP_STAT);
	aftr_wakeup_stat[aftr_wakeup_count].wakeup_stat1 = __raw_readl(EXYNOS5433_WAKEUP_STAT1);
	aftr_wakeup_stat[aftr_wakeup_count].wakeup_stat2 = __raw_readl(EXYNOS5433_WAKEUP_STAT2);
	aftr_wakeup_count++;
	if (aftr_wakeup_count >= EXYNOS_WAKEUP_STAT_BUF_SIZE)
		aftr_wakeup_count = 0;

	/* Clear wakeup state register */
	__raw_writel(0x0, EXYNOS5433_WAKEUP_STAT);
	__raw_writel(0x0, EXYNOS5433_WAKEUP_STAT1);
	__raw_writel(0x0, EXYNOS5433_WAKEUP_STAT2);
...
static void exynos_save_mif_dll_status(void)
{
	...
	if (tmp & (0x1 << 5)) {
		__raw_writel(0x1, EXYNOS5433_DREX_CALN2);
		processed_forcing = true;
	}
	else
		__raw_writel(0x0, EXYNOS5433_DREX_CALN2);
...
static void exynos_restore_mif_dll_status(void)
{
	...
	tmp = __raw_readl(regs_lpddrphy0 + 0xB0);
	if (__raw_readl(EXYNOS5433_DREX_CALN2))
		tmp |= (0x1 << 5);
	else
		tmp &= ~(0x1 << 5);
	__raw_writel(tmp, regs_lpddrphy0 + 0xB0);
				...
				int lp_mode, int index, int enter_mode)
{
	...
	__raw_writel(EXYNOS_CHECK_DIRECTGO, EXYNOS5433_DREX_CALN1);
...
early_wakeup:
	...
	/* This is W/A for gating Mclk during LPA/DSTOP */
	/* Clear flag  */
	__raw_writel(0, EXYNOS5433_DREX_CALN1);
	...
	lpa_wakeup_stat[lpa_wakeup_count].wakeup_stat = __raw_readl(EXYNOS5433_WAKEUP_STAT);
	lpa_wakeup_stat[lpa_wakeup_count].wakeup_stat1 = __raw_readl(EXYNOS5433_WAKEUP_STAT1);
	lpa_wakeup_stat[lpa_wakeup_count].wakeup_stat2 = __raw_readl(EXYNOS5433_WAKEUP_STAT2);
