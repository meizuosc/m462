...
/* The cpu is really powered off in trustzone, need to wait for power off in
 * platform_cpu_kill() --> exynos_cpu_kill() */
void exynos_power_down_cpu(unsigned int cpu)
{
#if defined(CONFIG_EXYNOS_CLUSTER_POWER_DOWN) || defined(CONFIG_ARM_TRUSTZONE)
	struct cpumask mask;
...
int __ref exynos_cpu_kill(unsigned int cpu)
{
	/* wait max 5 ms until cpu is off */
