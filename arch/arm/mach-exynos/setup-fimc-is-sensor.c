	...
	struct exynos_sensor_pin *pin_ctrls, int channel)
{
	...
	pr_debug("%s(pin(%d), act(%d), ch(%s), delay(%d), voltage(%d))\n", __func__, pin, act, ch_name, delay, voltage);
	...
	u32 enable)
{
	...
	pr_info("%s(), SENSOR_SCENARIO:%d, %s\n",
		__func__, scenario, enable ? "OFF": "ON");
	for (i = 0; i < GPIO_CTRL_MAX; ++i) {
		if (pin_ctrls[scenario][enable][i].act == PIN_END) {
			pr_debug("gpio cfg is end(%d)\n", i);
			break;
		}
		...
		ret = exynos_fimc_is_sensor_pin_control(pdev,
			...
			pdata->csi_ch);
		if (ret) {
			pr_err("%s(), err!exynos5_fimc_is_sensor_gpio(%d, %s, %d, %d) is fail(%d)",
				__func__,
