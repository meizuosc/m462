...
extern int exynos_fiq_get_pdata(struct platform_device *pdev, int id);
static int fiq_debugger_probe(struct platform_device *pdev)
{
	...
	/*exynos fiq pdata*/
	if (pdev->dev.of_node) {
		ret = exynos_fiq_get_pdata(pdev, CONFIG_S3C_LOWLEVEL_UART_PORT);
		if (ret < 0) {
			pr_err("%s: exynos_fiq_get_pdata failed!\n", __func__);
			return ret;
		}
	}
...
MODULE_DEVICE_TABLE(platform, fiq_debugger_match);
...
static struct platform_driver fiq_debugger_driver = {
	...
	.driver	= {
		...
		.of_match_table = fiq_debugger_match
	},
};
...
arch_initcall(fiq_debugger_init);
