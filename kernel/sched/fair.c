...
#ifdef CONFIG_BIG_SUPPRESSING_SCHED
/*
 * Migration Threshold shifting @ Low power mode (such as No Screen update)
 *
 * Up threhold for KFC -> EGL  : Suppressed
 * Dn threshold for EGL -> KFC : None -> locate the running job on EGL continuously.
 */
static int hmp_suppress_val = 0;

unsigned int hmp_up_threshold_lp = 1023;
unsigned int hmp_down_threshold_lp = 256;

unsigned int hmp_up_threshold_normal = 700;
unsigned int hmp_down_threshold_normal = 256;

#define SuppressingCore 0x3
static int hmp_suppressed_cpufreq = 0;
unsigned int  BigSupFreq = 1000000; // Default Hispeed Freq
#endif
...
static int hmp_busy_thread_boost;
...
#ifdef CONFIG_BIG_SUPPRESSING_SCHED
static inline int hmp_suppressed(void)
{
	int ret;
	unsigned long flags;

	raw_spin_lock_irqsave(&hmp_boost_lock, flags);
	if (hmp_suppress_val && hmp_suppressed_cpufreq)
		ret = 1;
	else
		ret = 0;
	raw_spin_unlock_irqrestore(&hmp_boost_lock, flags);
...
static bool inline fast_cpu_is_online(void);
...
static inline int hmp_semiboost(void)
{
	...
	if (timekeeping_suspended || !fast_cpu_is_online())
		return 0;

	now = ktime_to_us(ktime_get());

	raw_spin_lock_irqsave(&hmp_semiboost_lock, flags);
	if (hmp_semiboost_val || now < hmp_semiboostpulse_endtime)
		ret = 1;
	else
		ret = 0;
...
#ifdef CONFIG_BIG_SUPPRESSING_SCHED
		hmp_down_threshold_normal = value;
#endif
...
static int hmp_busy_thread_boost_from_sysfs(int value)
{
	hmp_busy_thread_boost = value;

	return 0;
}

#ifdef CONFIG_BIG_SUPPRESSING_SCHED
static int hmp_big_suppress_freq_from_sysfs(int value)
{
...
/* max value for threshold is 1024 */
static int hmp_up_threshold_lp_from_sysfs(int value)
{
	if ((value > 1024) || (value < 0))
		return -EINVAL;

	if (value <= hmp_down_threshold_lp)
		return -EINVAL;

	hmp_up_threshold_lp = value;

	return 0;
}

static int hmp_down_threshold_lp_from_sysfs(int value)
{
	if ((value > 1024) || (value < 0))
		return -EINVAL;

	if (value >= hmp_up_threshold_lp)
		return -EINVAL;

	hmp_down_threshold_lp = value;

	return 0;
}

static int hmp_suppress_from_sysfs(int value)
{
	unsigned long flags;
	int ret = 0;
	...
	if (value == 1) {
		...
		hmp_suppressed_cpufreq = 1;

		hmp_up_threshold = hmp_up_threshold_lp;
		hmp_down_threshold = hmp_down_threshold_lp;
	}
	else if (value == 0) {
		hmp_suppress_val = 0;

		hmp_suppressed_cpufreq = 0;

		hmp_up_threshold = hmp_up_threshold_normal;
		hmp_down_threshold = hmp_down_threshold_normal;
	}
	else {
		ret = -EINVAL;
	}
...
#endif // CONFIG_BUG_SUPPRESSING_SCHED
...
static int hmp_freqinvar_from_sysfs(int value)
{
	if (value < 0 || value > 1)
		return -1;
	hmp_data.freqinvar_load_scale_enabled = value;
	return 0;
}
#endif
...
static int hmp_attr_init(void)
{
	...
	hmp_attr_add("busy_thread_boost",
		&hmp_busy_thread_boost,
		NULL,
		hmp_busy_thread_boost_from_sysfs);
#ifdef CONFIG_BIG_SUPPRESSING_SCHED
	hmp_attr_add("big_suppressing_freq",
		&BigSupFreq,
		NULL,
		hmp_big_suppress_freq_from_sysfs);

	hmp_attr_add("up_threshold_lp",
		&hmp_up_threshold_lp,
		NULL,
		hmp_up_threshold_lp_from_sysfs);

	hmp_attr_add("down_threshold_lp",
		&hmp_down_threshold_lp,
		NULL,
		hmp_down_threshold_lp_from_sysfs);

	hmp_attr_add("big_suppress",
		&hmp_suppress_val,
		NULL,
		hmp_suppress_from_sysfs);
#endif // CONFIG_BIG_SUPPRESSING_SCHED
...
static int hmp_is_family_in_fastest_domain(struct task_struct *p)
{
	...
	list_for_each_entry(thread_p, &p->thread_group, thread_group) {
		...
		//pr_info("is_family_in_fastest_domain 1: %d: %d\n", p->pid, thread_p->pid);
		if (thread_se->avg.load_avg_ratio >= hmp_up_threshold &&
				hmp_cpu_is_fastest(task_cpu(thread_p))) {
			//pr_info("is_family_in_fastest_domain: %d\n", thread_p->pid);
			return thread_p->pid;
		}
	}
	return 0;
...
#ifdef CONFIG_SCHED_HMP
	if (hmp_busy_thread_boost > 0 && hmp_cpu_is_slowest(prev_cpu) && p->parent && p->parent->pid > 2) {
		if ((thread_pid = hmp_is_family_in_fastest_domain(p->group_leader)) != 0) {
			...
			if (new_cpu != NR_CPUS) {
				//pr_info("hmp_up_migration: %d, parent=%d, from=%d, to=%d, by=%d\n", p->pid, p->parent->pid, prev_cpu, new_cpu, thread_pid);
				hmp_next_up_delay(&p->se, new_cpu);
				return new_cpu;
			}
			/* failed to perform HMP balance, use normal balance */
			new_cpu = cpu;
		}
	}
#endif
...
static bool fast_cpu_is_online(void)
{
...
static unsigned int hmp_up_migration(int cpu, int *target_cpu, struct sched_entity *se)
{
	...
	if (hmp_cpu_is_fastest(cpu) || !fast_cpu_is_online())
		return 0;
...
static unsigned int hmp_down_migration(int cpu, struct sched_entity *se)
{
	...
	if (hmp_cpu_is_slowest(cpu) || !fast_cpu_is_online())
		return 0;
...
#ifdef CONFIG_BIG_SUPPRESSING_SCHED
	if (hmp_suppressed()) {
		// DO NOT Forced up migration @ suppressed condition
	} else {
					...
					unsigned long val, void *data)
{
	...
	{
		unsigned long flags;
		if((freq->cpu & SuppressingCore) &&
				(hmp_suppress_val)){ // It's a LITTLE, Suppressed, but High load
			if(freq->new > BigSupFreq) {
				raw_spin_lock_irqsave(&hmp_boost_lock, flags);
				hmp_suppressed_cpufreq = 0;
				hmp_up_threshold = hmp_up_threshold_normal;
				hmp_down_threshold = hmp_down_threshold_normal;
