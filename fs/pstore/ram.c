...
#ifdef CONFIG_OF
static unsigned long g_ramoops_phy_addr;
static int __init ramoops_setup(char *str)
{
	unsigned long base = 0, size =0;
	struct device_node *np = NULL;

	np = of_find_node_by_path("/ramoops");
	if (!np) {
		pr_err("Can't find ramoops node!\n");
	}

	of_property_read_u32(np, "ramoops,mem_address", (int*)&base);
	of_property_read_u32(np, "ramoops,mem_size", (int*)&size);

	if (!base || !size)
		goto out;

	if (!(reserve_bootmem(base, size, BOOTMEM_EXCLUSIVE))) {
		g_ramoops_phy_addr = base;
		pr_info("memory reserved - base: 0x%08X, size: 0x%08X\n",
				(unsigned int)base, (unsigned int)size);
		return 0;
	}
out:
	pr_err("ramoops: buffer reserved failed base:%08X, size:%08X\n",
			(unsigned int)base, (unsigned int)size);
	return -EINVAL;
}
__setup("ramoops_setup", ramoops_setup);

static struct ramoops_platform_data *ramoops_dt_platform_data(struct device *dev)
{
	int error;
	struct ramoops_platform_data *pdata;
	struct device_node *np = dev->of_node;

	if (!of_device_is_compatible(np, "linux,ramoops")) {
		error = -ENODEV;
                goto err_out;
	}
	...
	pdata->mem_address = g_ramoops_phy_addr;
...
static int ramoops_probe(struct platform_device *pdev)
{
	...
	if (!pdata)
		pdata = ramoops_dt_platform_data(dev);
	...
	if (cxt->max_dump_cnt)
		goto fail_out;
	if (!pdata->mem_address) {
		pr_err("The memory address must be non-zero\n");
		goto fail_out;
	}
...
MODULE_DEVICE_TABLE(of, ramoops_match);
