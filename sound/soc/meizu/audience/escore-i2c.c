...
#if 0
// if err means that dsp crash, save log to file
if (err) {
	{
		int i=0;
		char cmd_history[128];
		struct file *fp;
		mm_segment_t fs;
		loff_t pos;
		int index = 0;
		...
		printk("---------------------->\n");
		printk("DSP read/write failed! Recording command history...\n");
		printk("---------------------->\n");

		memset(cmd_history, 0x0, sizeof(cmd_history));

		fp = filp_open("/data/es705_command_history.txt", O_RDWR | O_CREAT, 0644);
		if (IS_ERR(fp)) {
			printk("es705_command_history create failed\n");
			return -1;
		}
		...
		for (index = 0; index < ES_MAX_ROUTE_MACRO_CMD; index++) {
			i = index + cmd_hist_index;
			i %= ES_MAX_ROUTE_MACRO_CMD;

			if (cmd_hist[i].cmd != 0) {
				snprintf(cmd_history, sizeof(cmd_history),
					"[%5lu.%03lu] cmd=0x%04x 0x%04x; resp=0x%04x 0x%04x;\n",
					cmd_hist[i].timestamp.tv_sec,
					cmd_hist[i].timestamp.tv_nsec / (1000*1000),
					cmd_hist[i].cmd >> 16,
					cmd_hist[i].cmd & 0xffff,
					cmd_hist[i].resp >> 16,
					cmd_hist[i].resp & 0xffff);
				vfs_write(fp, cmd_history, sizeof(cmd_history), &pos);
			}
			else
				memset(cmd_history, 0x0, sizeof(cmd_history));
		}
		...
		printk("<---------------------\n");
		printk("DSP command history finish...\n");
		printk("<---------------------\n");
	}

	{
		char *argv[2];
		static char *reg_dump = "/data/es705_reg_dump.sh";
		static char *envp[] = {"HOME=/", "TERM=linux", "PATH=/usr/bin:/bin", NULL};
		int ret;

		printk("---------------------->\n");
		printk("DSP read/write failed! Dump DSP PCM and memory...\n");
		printk("---------------------->\n");

		// reset dsp and download firmware
		escore->mode = SBL;
		escore_gpio_reset(escore);
		//disable pm runtime
		escore_pm_disable();

		// execute script to dump dsp log
		argv[0] = reg_dump;
		argv[1] = NULL;
		ret = call_usermodehelper(reg_dump, argv, envp, UMH_WAIT_EXEC);
		if (ret)
			printk("%s: call_usermodehelper failed, ret=%d\n", __func__, ret);

		printk("<---------------------\n");
		printk("DSP dump finish...\n");
		printk("<---------------------\n");
	}
}
#endif
