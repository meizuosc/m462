...
vs_sleep_err:
		if (rc) {
			dev_err(escore->dev, "%s(): Chip dead and will reset....\n\n",
				__func__);
				...
				const char *vs_filename)
{
	...
	return escore_request_firmware((const struct firmware **)&voice_sense->vs,
			      vs_filename, escore->dev);
}
...
int escore_vs_request_bkg(struct escore_priv *escore, const char *bkg_filename)
{
	...
	rc = escore_request_firmware((const struct firmware **)&voice_sense->bkg,
			bkg_filename, escore->dev);
	if (rc) {
...
void escore_vs_release_bkg(struct escore_priv *escore)
{
	...
	escore_release_firmware(voice_sense->bkg);
}
...
request_firmware_kw_exit:
	while (i-- > 0) {
		...
		escore_release_firmware(voice_sense->kw[i]);
	}
	return  rc;
}
