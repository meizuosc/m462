...
#define ES705_FILE_FW  "audience/es705/audience-es705-fw.bin"
#define ES705_FILE_VS  "audience/es705/audience-es705-vs.bin"
#define ES705_FILE_BKG "audience/cvqmodels/adnc_cvq_detection_bkg_w_hdrs.bin"
...
//	int retry = 2;
	...
	// NOTE: try to fix noise in voice call, it is only a workaround.
//	for ( ; retry > 0; --retry) {
				...
				unsigned int set_power_state_cmd)
{
	...
	while (next_power_state != escore->escore_power_state) {
		...
		case ES_SET_POWER_STATE_NORMAL:
			...
			if (next_power_state == ES_SET_POWER_STATE_VS_OVERLAY) {
				u32 cmd, rsp;
			...
			if (escore->escore_power_state ==
				...
				do {
					...
					if (rc)
						pr_warn("%s(): escore_vs_load failed, retry...\n", __func__);
					else
						break;
				} while (retry--);

				if (rc) {
					pr_err("%s(): vs fw downlaod failed\n", __func__);
					goto es705_power_transition_exit;
				}
			}
			break;
		case ES_SET_POWER_STATE_VS_OVERLAY:
			...
			rc = escore_write(NULL, set_power_state_cmd,
					escore->escore_power_state);
			if (rc) {
				pr_err("%s(): Power state cmd write failed\n",
						__func__);
				escore->escore_power_state =
					ES_SET_POWER_STATE_VS_OVERLAY;
				goto es705_power_transition_exit;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(escore_priv.dev, "%s(): Current state:%s, Requested state:%s\n",
			__func__,
			power_state_cmd[escore_priv.escore_power_state],
			power_state_cmd[value]);
	...
	if (rc) {
		dev_err(escore_priv.dev,
			"%s(): es705_power_transition() failed\n",
			__func__);
	}
...
irqreturn_t es705_cmd_completion_isr(int irq, void *data)
{
	...
	if (gpio_value) {
		...
		pr_debug("%s(): API Falling edge completion\n",	__func__);
...
irqreturn_t es705_irq_work(int irq, void *data)
{
	...
	do {
		msleep(30);
		count++;
	} while ((escore->dev->power.disable_depth != 0)  && (count < 10));
	cmd = ES_GET_EVENT << 16;
...
static struct esxxx_platform_data *es705_populate_dt_pdata(struct device *dev)
{
	...
	ret = of_property_read_u32(dev->of_node, "adnc,ext_clk_rate", &pdata->ext_clk_rate);
	if (ret) {
		dev_err(dev, "%s(): get ext_clk_rate failed\n", __func__);
		goto dt_populate_err;
	}
	dev_dbg(dev, "%s(): ext_clk_rate %d\n", __func__, pdata->ext_clk_rate);
#endif
...
/* API Interrupt registration */
...
#ifndef CONFIG_SND_SOC_ES_WAKEUP_UART
	...
	ret = of_property_read_u32(dev->of_node, "adnc,hs_uart_baud", &pdata->hs_uart_baud);
	if (ret)
		pdata->hs_uart_baud = 3000000;
	dev_dbg(dev, "%s(): hs_uart_baud %d\n", __func__, pdata->hs_uart_baud);
...
*/
	...
	wake_lock_init(&escore_priv.escore_wakelock,
			WAKE_LOCK_SUSPEND, "escore_wakelock");
	...
	//		      ES705_FILE_FW, escore_priv.dev);
	rc = escore_request_firmware((const struct firmware **)&escore_priv.standard,
			ES705_FILE_FW, escore_priv.dev);
	if (rc) {
		dev_err(escore_priv.dev, "%s(): request_firmware(%s) failed %d\n",
			__func__, ES705_FILE_FW, rc);
		goto request_firmware_error;
	}

	escore_vs_init(&escore_priv);

	//rc = escore_vs_request_firmware(&escore_priv, ES705_FILE_VS);
	rc = escore_vs_request_firmware(&escore_priv, ES705_FILE_VS);
	if (rc) {
		dev_err(escore_priv.dev,
			"%s(): request_firmware(%s) failed %d\n",
			__func__, ES705_FILE_VS, rc);
		goto request_vs_firmware_error;
	}

	//rc = escore_vs_request_bkg(&escore_priv, ES705_FILE_BKG);
	rc = escore_vs_request_bkg(&escore_priv, ES705_FILE_BKG);
	if (rc) {
		dev_err(escore_priv.dev,
	...
	if (pdata->gpioa_gpio != -1) {
		...
		rc = request_threaded_irq(gpio_to_irq(pdata->gpioa_gpio), NULL,
				es705_cmd_completion_isr, irq_flags,
	...
	if (pdata->gpiob_gpio != -1) {
		rc = request_threaded_irq(gpio_to_irq(pdata->gpiob_gpio),
					  NULL,
					  es705_irq_work, IRQF_TRIGGER_RISING | IRQF_ONESHOT,
...
request_vs_firmware_error:
	...
	escore_release_firmware(escore_priv.standard);
...
static __exit void es705_exit(void)
{
	...
	if (escore_priv.fw_requested) {
		...
		escore_release_firmware(escore_priv.standard);
		escore_release_firmware(voice_sense->vs);
	}
#if defined(CONFIG_SND_SOC_ES_UART_STREAMDEV)
