...
#define FW_FILE		"es515_fw"
#undef USE_BUILTIN_FW
...
static struct es515_route_configs es515_def_route_configs[] = {
	...
	/* Sinewave disabled */
	{
		.macro = {
			...
			0x90, 0x4c, 0x00, 0x01,
			/* preset mode 62:
			   16bits 48KHz speaker playback + MIC1 capture */
			0x80, 0x31, 0x00, 0x3E,
			...
			/* -20 dB */
			0x90, 0x0D, 0x00, 0x09,
			/* set device param Id HF Left Gain */
			0x90, 0x0C, 0x1D, 0x00,
			/* +20 dB */
			0x90, 0x0D, 0x00, 0x1F,
			/* set device param Id HF Right Gain */
			0x90, 0x0C, 0x1D, 0x03,
			/* +20 dB */
			0x90, 0x0D, 0x00, 0x1F,
...
/*		.active_out_codec_port = ES515_CODEC_PORT_SPEAKER,*/
	...
	/* [5] Route for two capture */
	{
		.macro = {
			...
			0x90, 0x4c, 0x00, 0x01,
			/* preset mode 78:
			   16bits 48KHz headphone playback + Headphone MIC capture */
			0x80, 0x31, 0x00, 0x4E,
#ifndef CONFIG_ES515_MASTER	/* Codec Slave */
			/* Set param ID for  Port 0, clock control */
			0x80, 0x0C, 0x0A, 0x09,
...
/*		.active_out_codec_port = ES515_CODEC_PORT_SPEAKER,*/
	...
	/* [6] Route for two Playback */
	{
		.macro = {
			...
			0x90, 0x4c, 0x00, 0x01,
			/* preset mode 79:
				16bits 48KHz speaker & headphone playback + Headphone MIC capture */
			0x80, 0x31, 0x00, 0x4F,
#ifndef CONFIG_ES515_MASTER	/* Codec Slave */
			/* Set param ID for  Port 0, clock control */
			0x80, 0x0C, 0x0A, 0x09,
...
/*		.active_out_codec_port = ES515_CODEC_PORT_SPEAKER,*/
	...
	/* [7] Route for Playback + Capture */
	{
		.macro = {
			...
			0x90, 0x4c, 0x00, 0x01,
			/* preset mode 80:
			16bits 48KHz speaker & headphone playback + MIC1 capture */
			0x80, 0x31, 0x00, 0x50,
#ifndef CONFIG_ES515_MASTER	/* Codec Slave */
			/* Set param ID for  Port 0, clock control */
			0x80, 0x0C, 0x0A, 0x09,
...
static int es515_sleep(struct es515_priv *es515)
{
	...
	/* send Smooth Mute, 0x904e 0000 */
	memset(msg, 0, 16);
	msg[0] = ES515_SET_SMOOTH >> 8;
	msg[1] = ES515_SET_SMOOTH & 0xFF;
	msg[2] = ES515_SET_SMOOTH_RATE >> 8;
	msg[3] = ES515_SET_SMOOTH_RATE & 0xFF;
	rc = ES515_BUS_WRITE(es515, msg, 4, 1);
	if (rc < 0) {
		pr_err("%s() : failed smooth mute write.\n", __func__);
		return rc;
	}
	usleep_range(10000, 11000);

	/* send sleep command, 0x8010  0001 */
	memset(msg, 0, 16);
	msg[0] = ES515_SET_POWER_STATE >> 8;
	msg[1] = ES515_SET_POWER_STATE & 0xFF;
	msg[2] = ES515_SET_POWER_STATE_SLEEP >> 8;
	msg[3] = ES515_SET_POWER_STATE_SLEEP & 0xFF;
	rc = ES515_BUS_WRITE(es515, msg, 4, 1);
	if (rc < 0) {
		pr_err("%s() : failed sleep command write.\n", __func__);
		return rc;
	}
...
static int es515_codec_resume(struct snd_soc_codec *codec)
{
	...
	es515_switch_route(es515_internal_route_num);
	pr_info("%s()\n", __func__);
#else
	es515_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
#endif
...
int es515_jack_assign(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_jack_new(codec, "es515_jack", SND_JACK_HEADSET, jack);
	if (ret != 0) {
...
EXPORT_SYMBOL_GPL(es515_jack_remove);
#endif
