...
static int nl80211_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (rdev->scan_req) {
		pr_err("%s(), err! iftype:%d, netdev->name:%s, rdev->scan_req is:0x%p, NOT NULL!!!\n",
			__func__, wdev->iftype, wdev->netdev->name ,rdev->scan_req);
		err = -EBUSY;
		goto unlock;
	}
 ...
 unlock:
	#if 0
	if (unlikely(err)) {
		pr_err("%s(), iftype:%d, netdev->name:%s, returns err %d, for test, dump stack\n",
			__func__, wdev->iftype, wdev->netdev->name, err);
		dump_stack();
	}
	#endif
