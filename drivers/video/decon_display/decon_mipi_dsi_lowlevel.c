/* linux/drivers/video/decon_display/decon_mipi_dsi_lowlevel.c
 *
...
void s5p_mipi_dsi_display_config(struct mipi_dsim_device *dsim)
{
	...
	/* main lcd */
	reg |= ((u8) (g_dsim_config.e_burst_mode) & 0x3) << 26 |
		((u8) (g_dsim_config.e_virtual_ch) & 0x3) << 18 |
		((u8) (g_dsim_config.e_pixel_format) & 0x7) << 12;
...
{
	unsigned int reg, old , mask = 0;
	old = readl(dsim->reg_base + S5P_DSIM_PHYCTRL);
	mask = (1 << 30) | (0x3ff << 0);

	reg = (ulpsexitctrl & mask) | ((1 << 30) & mask) | (old & ~mask);

	writel(reg, dsim->reg_base + S5P_DSIM_PHYCTRL);
}

void s5p_mipi_dsi_set_m_dphyctrl(struct mipi_dsim_device *dsim)
{
	unsigned int reg, old , mask = 0;
	old = readl(dsim->reg_base + S5P_DSIM_M_PHYCTRL);
	mask = (1 << 30);

	reg = ((1 << 30) & mask) | (old & ~mask);

	writel(reg, dsim->reg_base + S5P_DSIM_M_PHYCTRL);
}

void s5p_mipi_dsi_set_m_pllctrl(struct mipi_dsim_device *dsim,
	unsigned int m_pllctl)
{
	unsigned int reg, old , mask = 0;
	old = readl(dsim->reg_base + S5P_DSIM_M_PLLCTRL);
	mask = (0xf << 15) | (1 << 19) | (0x3 << 2);

	reg = ((m_pllctl << 15) & mask) | ((1 << 19) & mask) | ((0x3 << 2) & mask)| (old & ~mask);

	writel(reg, dsim->reg_base + S5P_DSIM_M_PLLCTRL);
}
