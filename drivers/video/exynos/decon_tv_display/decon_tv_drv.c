			...
			unsigned long arg)
{
	...
	if (boot_time) {
		...
		if (timecnt == 0)
			...
			dev_info(dex->dev, "DECON-TV stopped to clear the boot logo.");
...
#ifdef CONFIG_ESPRESSO_TV_PRIMARY
int tv_wait_for_vsync_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		int ret = 0;
		ret = wait_event_interruptible(dex->vsync_wait,
				!ktime_equal(timestamp, dex->vsync_timestamp));

		if (!ret)
			sysfs_notify(&dex->dev->kobj, NULL, "tv_vsync");
        }

        return 0;
}

static ssize_t tv_vsync_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
       ...
       struct dex_device *dex = platform_get_drvdata(pdev);
        ...
        return scnprintf(buf, PAGE_SIZE, "%llu\n",
                        ktime_to_ns(dex->vsync_timestamp));
}

static DEVICE_ATTR(tv_vsync, S_IRUGO, tv_vsync_show, NULL);
#endif
...
#ifdef CONFIG_ESPRESSO_TV_PRIMARY
	ret = device_create_file(dev, &dev_attr_tv_vsync);
	if (ret)
		goto fail_entity;

	dex->vsync_thread = kthread_run(tv_wait_for_vsync_thread,
			dex, DEX_DRIVER_NAME);

	if (IS_ERR(dex->vsync_thread)) {
		ret = PTR_ERR(dex->vsync_thread);
		dex->vsync_thread = NULL;
		dex_err("failed to run tv vsync thread\n");
	}
#endif
	return 0;
...
#ifdef CONFIG_ESPRESSO_TV_PRIMARY
        if (dex->vsync_thread)
                kthread_stop(dex->vsync_thread);
        device_remove_file(dev, &dev_attr_tv_vsync);
#endif
