...
static int dsim_reg_calculate_pms(struct dsim_pll_param *pll)
{
	 ...
	 */
	dsim_info("requested HS clock is %u\n", pll->pll_freq);
...
calculation_success:
	...
	pll->m = m_div;
	pll->s = s_div;
	pll->pll_freq = fout_cal * 2;
	dsim_info("calculated HS clock is %u Mhz. p(%u), m(%u), s(%u)\n",
...
static int dsim_reg_get_dphy_timing(u32 hs_clk, u32 esc_clk, struct dphy_timing_value *t)
{
	...
	if ((esc_clk > 20) || (esc_clk < 7)) {
		dsim_err("%u Mhz can't be used as escape clock\n", esc_clk);
		return -EINVAL;
	}

	i = sizeof(b_dphyctl) / sizeof(b_dphyctl[0]) - 1;
	while (i) {
		if (b_dphyctl[i][0] < esc_clk) {
			i--;
			continue;
		} else {
	...
	if (!i) {
		dsim_err("%u esc clock can't find proper dphy timing values\n", esc_clk);
		return -EINVAL;
	}
