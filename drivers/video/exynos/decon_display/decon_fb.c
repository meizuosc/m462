...
#if defined(CONFIG_ENG_KERNEL) || defined(CONFIG_OVERSEAS_KERNEL) || defined(CONFIG_RECOVERY_KERNEL) || defined(CONFIG_USER_KERNEL) || defined(CONFIG_FACTORY_KERNEL)
#include "mz_logo.h"
#elif defined(CONFIG_UNICOM_KERNEL)
#include "mz_unicom_logo.h"
#else
#error "please define logo type!"
#endif
#endif
...
static int s3c_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_UNBLANK:
		...
		if (dsim_for_decon->power_supply_flags == POWER_SUPPLY_TYPE_1) {
			usleep_range(32000, 32100);
		}
...
#ifndef CONFIG_ESPRESSO_TV_PRIMARY
		if (enabled)
...
#ifdef CONFIG_FB_HIBERNATION_DISPLAY
extern unsigned int frame_done_count;
extern unsigned int te_count;
static ssize_t s3c_fb_pm_info(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct display_driver *dispdrv;
	...
	return scnprintf(buf, PAGE_SIZE, "clock_enabled: %s \
		\nlock_count: %d \
		\nclk_idle_count: %d \
		\npwr_idle_count: %d \
		\noutput_on: %s \
		...
		\nframe_cnt: %d \
		\nte_cnt: %d\n",
		(dispdrv->pm_status.clock_enabled == 1 ? "enable" : "disable"),
		atomic_read(&dispdrv->pm_status.lock_count),
		dispdrv->pm_status.clk_idle_count,
		dispdrv->pm_status.pwr_idle_count,
		(dispdrv->decon_driver.sfb->output_on ? "on" : "off"),
		(dispdrv->decon_driver.sfb->power_state == POWER_ON ? "POWER_ON" : 
		(dispdrv->decon_driver.sfb->power_state == POWER_DOWN ? "POWER_DOWN" : 
		(dispdrv->decon_driver.sfb->power_state == POWER_HIBER_DOWN ? "POWER_HIBER_DOWN" : "UNKNOW_STATE"))),
		(dispdrv->platform_status == DISP_STATUS_PM0 ? "PM0" :
		(dispdrv->platform_status == DISP_STATUS_PM1 ? "PM1" :
		(dispdrv->platform_status == DISP_STATUS_PM2 ? "PM2" : "UNKNOW_STATE"))),
		ktime_to_ns(dispdrv->decon_driver.sfb->vsync_info.timestamp),
		frame_done_count,
		te_count);
}

static DEVICE_ATTR(pm_info, S_IRUGO, s3c_fb_pm_info, NULL);
#endif
...
#if defined(CONFIG_UNICOM_KERNEL)
	unsigned int row_offset = ((yres < PIC_HEIGHT) ? 0 : ((yres - PIC_HEIGHT) >> 1));
#else
...
#if defined(CONFIG_UNICOM_KERNEL)
	memset(curBufBase, 0xff, xres * 4 * yres);
#else
	memset(curBufBase, 0, xres * 4 * yres);
#endif
...
int create_decon_display_controller(struct platform_device *pdev)
{
...
#ifndef CONFIG_DECON_DISPLAY_LOGO
	decon_reg_set_winmap(default_win, 0x000000 /* black */, 1);
#else
	s3c_fb_copy_logo(fbinfo->var.xres, fbinfo->var.yres, fbinfo->screen_base);
#endif
...
#ifdef CONFIG_FB_HIBERNATION_DISPLAY
	ret = device_create_file(sfb->dev, &dev_attr_pm_info);
	if (ret) {
		dev_err(sfb->dev, "failed to create pm info file\n");
		goto err_create_pm_info_file;
	}
#endif
	...
	dev_warn(sfb->dev, "[Info]: Succeed to load decon fb!\r\n");
...
err_iovmm:
#ifdef CONFIG_FB_HIBERNATION_DISPLAY
	device_remove_file(sfb->dev, &dev_attr_pm_info);

err_create_pm_info_file:
#endif
