...
int create_mipi_dsi_controller(struct platform_device *pdev)
{
	...
	if (dsim->dsim_lcd_drv->read_id) {
		gpio = dispdrv->dt_ops.get_display_dsi_reset_gpio();
		dsim->dsim_lcd_drv->read_id(dsim, gpio->id[DECON_DIS_GPIO_LCD_ID]);
	}

#if defined(CONFIG_DECON_LCD_M6X) || defined(CONFIG_DECON_LCD_M7X)
	dispdrv->dt_ops.init_display_dsi_regulator(dsim->power_supply_flags);
#endif
	...
	if (dsim->dsim_lcd_drv->read_ver) {
		dsim->dsim_lcd_drv->read_ver(dsim);
	}
