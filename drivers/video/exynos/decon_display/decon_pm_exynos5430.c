...
int init_display_decon_clocks(struct device *dev)
{
	...
	else if (lcd->xres * lcd->yres == 1080 * 1800)
		exynos_display_set_rate(dev, "disp_pll", 142 * MHZ);
	...
	else if (lcd->xres * lcd->yres == 1536 * 2560) {
		if (lcd->fps == 60)
			exynos_display_set_rate(dev, "disp_pll", 267 * MHZ);
		else if (lcd->fps == 58)
			exynos_display_set_rate(dev, "disp_pll", 258 * MHZ);
	} else
...
int enable_display_driver_power(struct device *dev)
{
...
#if !defined(CONFIG_MACH_XYREF5430) && !defined(CONFIG_MACH_ESPRESSO5433) && !defined(CONFIG_MACH_ESPRESSO5430) && !defined(CONFIG_MACH_M76)
	struct display_driver *dispdrv;
	...
	struct display_driver *dispdrv;
	struct display_gpio *gpio;
	struct display_regulator *regulator;
	int id, ret = 0;

	dispdrv = get_display_driver();

	regulator = dispdrv->dt_ops.get_display_dsi_regulator();

	gpio = dispdrv->dt_ops.get_display_dsi_reset_gpio();
	id = gpio_request(gpio->id[DECON_DIS_GPIO_RESET], "lcd_reset");
	if (id < 0) {
		pr_err("Failed to get gpio number for the lcd power\n");
		return -EINVAL;
	}
	gpio_direction_output(gpio->id[DECON_DIS_GPIO_RESET], 0x01);

#if defined(CONFIG_DECON_LCD_M6X)
	gpio_set_value(gpio->id[DECON_DIS_GPIO_RESET], 0);
	ret = regulator_enable(regulator->vdd18);
	usleep_range(5000,6000);

	ret = regulator_enable(regulator->vsp);
	ret = regulator_enable(regulator->vsn);
	usleep_range(5000,6000);

	gpio_set_value(gpio->id[DECON_DIS_GPIO_RESET], 1);
	usleep_range(10000, 12000);
#elif defined(CONFIG_DECON_LCD_M7X)
	if (dispdrv->dsi_driver.dsim->power_supply_flags == POWER_SUPPLY_TYPE_0) {
		gpio_set_value(gpio->id[DECON_DIS_GPIO_RESET], 0);
		ret = regulator_enable(regulator->vdd18);
		...
		ret = regulator_enable(regulator->vcc);
		usleep_range(2000, 3000);

		ret = regulator_enable(regulator->nvcc);
		usleep_range(2000, 3000);

		ret = regulator_enable(regulator->vsn);
		usleep_range(1010,1020);
	...
	gpio_free(gpio->id[DECON_DIS_GPIO_RESET]);
#endif
...
#if defined(CONFIG_DECON_LCD_M6X)
	regulator_disable(regulator->vsn);
	regulator_disable(regulator->vsp);
	usleep_range(10000, 12000);
	
	gpio_set_value(gpio->id[DECON_DIS_GPIO_RESET], 0);
	regulator_disable(regulator->vdd18);
#elif defined(CONFIG_DECON_LCD_M7X)
	if (dispdrv->dsi_driver.dsim->power_supply_flags == POWER_SUPPLY_TYPE_0) {
		gpio_set_value(gpio->id[DECON_DIS_GPIO_RESET], 0);
		usleep_range(2000, 3000);
		...
		regulator_disable(regulator->nvcc);
		usleep_range(2000, 3000);

		regulator_disable(regulator->vcc);
		usleep_range(2000, 3000);
