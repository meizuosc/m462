...
#elif defined(CONFIG_DECON_LCD_M6X)
	.dsim_ddi_pd = &m6x_mipi_lcd_driver,
#elif defined(CONFIG_DECON_LCD_M7X)
	.dsim_ddi_pd = &m7x_mipi_lcd_driver,
#else
#error "Please Define Lcd Panel!"
#endif
};
...
#if defined(CONFIG_DECON_LCD_M6X) || defined(CONFIG_DECON_LCD_M7X)
static struct display_regulator g_disp_regulator;
#endif
...
#if defined(CONFIG_DECON_LCD_M6X) || defined(CONFIG_DECON_LCD_M7X)
struct display_regulator *get_display_dsi_regulator(void)
{
	return &g_disp_regulator;
}

int init_display_dsi_regulator(int power_supply_type)
{
	g_disp_regulator.vdd18 = regulator_get(NULL, "vdd18_lcd");
	if (IS_ERR(g_disp_regulator.vdd18)) {
		pr_err("lcd vdd18 regulator is not available.\n");
		return -EINVAL;
	}

	g_disp_regulator.vsp = regulator_get(NULL, "lcd-vsp-5v");
	if (IS_ERR(g_disp_regulator.vsp)) {
		pr_err("lcd VSP regulator is not available.\n");
		return -EINVAL;
	}

	g_disp_regulator.vsn = regulator_get(NULL, "lcd-vsn-5v");
	if (IS_ERR(g_disp_regulator.vsn)) {
		pr_err("lcd VSN regulator is not available.\n");
		return -EINVAL;
	}

#ifdef CONFIG_DECON_LCD_M7X
	if (power_supply_type == POWER_SUPPLY_TYPE_0) {
		g_disp_regulator.vcc = regulator_get(NULL, "lcd-vcc-5v");
		if (IS_ERR(g_disp_regulator.vsp)) {
			pr_err("lcd vcc regulator is not available.\n");
			return -EINVAL;
		}

		g_disp_regulator.nvcc = regulator_get(NULL, "lcd-nvcc-5v");
		if (IS_ERR(g_disp_regulator.vsn)) {
			pr_err("lcd nvcc regulator is not available.\n");
			return -EINVAL;
		}
	} else {
		g_disp_regulator.vcc = NULL;
		g_disp_regulator.nvcc = NULL;
	}
#endif
...
int parse_lcd_drvdata(struct device_node *np)
{
	...
	g_decon_lcd.xres = res[1];
	g_decon_lcd.yres = res[0];
#else
	...
	g_decon_lcd.xres = 1080;
	g_decon_lcd.yres = 1800;
	g_decon_lcd.width = 66;
	g_decon_lcd.height = 110;
	g_decon_lcd.hbp = 42;
	g_decon_lcd.hfp = 100;
	g_decon_lcd.hsa = 4;
	g_decon_lcd.vbp = 7;
	g_decon_lcd.vfp = 4;
	g_decon_lcd.vsa = 2;
	g_decon_lcd.hs_clk = 900;
	g_decon_lcd.mic = 0;
#elif defined(CONFIG_DECON_LCD_M7X)
	g_decon_lcd.xres = 1536;
	g_decon_lcd.yres = 2560;
	g_decon_lcd.width = 72;
	g_decon_lcd.height = 120;
	g_decon_lcd.hbp = 1;
	g_decon_lcd.hfp = 1;
	g_decon_lcd.hsa = 1;
	g_decon_lcd.vbp = 13;
