...
int hdmi_clk_set_rate(struct device *dev, const char *conid, unsigned int rate)
{
	struct clk *target;
...
void hdmi_clock_change(struct hdmi_device *hdev, int on)
{
	...
	if (on) {
		hdmi_clk_set_parent(dev, "mout_sclk_decon_tv_eclk_a", "mout_bus_pll_div2");
		hdmi_clk_set_parent(dev, "mout_sclk_decon_tv_eclk_b", "mout_sclk_decon_tv_eclk_a");
		hdmi_clk_set_parent(dev, "mout_sclk_decon_tv_eclk_c", "mout_sclk_decon_tv_eclk_b");
		hdmi_clk_set_parent(dev, "dout_sclk_decon_tv_eclk", "mout_sclk_decon_tv_eclk_c");

		hdmi_clk_set_rate(dev, "dout_sclk_decon_tv_eclk", 413 * 1000000);

		hdmi_clk_set_parent(dev, "sclk_decon_tv_eclk_disp", "dout_sclk_decon_tv_eclk");
		hdmi_clk_set_parent(dev, "mout_sclk_decon_tv_eclk_user", "sclk_decon_tv_eclk_disp");
		hdmi_clk_set_parent(dev, "mout_sclk_decon_tv_eclk", "mout_sclk_decon_tv_eclk_user");
		hdmi_clk_set_parent(dev, "dout_sclk_decon_tv_eclk_disp", "mout_sclk_decon_tv_eclk");

		hdmi_clk_set_rate(dev, "dout_sclk_decon_tv_eclk_disp", 413 * 1000000);
...
static void hdmi_hpd_changed(struct hdmi_device *hdev, int state)
{
	...
	switch_set_state(&hdev->hpd_audio_switch, state &&
			(edid_audio_informs(hdev) & AUDIO_CHANNEL_MASK) ? 1 : 0);
...
static int hdmi_probe(struct platform_device *pdev)
{
	...
	ret = switch_dev_register(&hdmi_dev->hpd_audio_switch);
	if (ret) {
		dev_err(dev, "request switch class failed.\n");
		goto fail_switch;
	}
