	...
	if (on) {
		...
		if (cam_qos > 0) {
			if (device->request_cam_qos == false) {
				pm_qos_add_request(&exynos_sensor_qos_cam, PM_QOS_DEVICE_THROUGHPUT, cam_qos);
		...
		if (int_qos > 0) {
			if (device->request_int_qos == false) {
				pm_qos_add_request(&exynos_sensor_qos_int, PM_QOS_CAM_THROUGHPUT, int_qos);
	...
	u32 cfgs, u32 width, u32 height, u32 framerate)
{
	...
	if (idx < 0)
		err("could not find proper sensor mode: %dx%d@%dfps, return %d",
			width, height, framerate, mode);
	else
		pr_info("%s(), sensor mode(%dx%d@%d) = %d, return %d\n",
			__func__,
			cfg[idx].width,
			cfg[idx].height,
			cfg[idx].framerate,
...
static int fimc_is_sensor_gpio_on(struct fimc_is_device_sensor *device)
{
	...
	#if defined(CONFIG_MEIZU_CAMERA_SPECIAL)
	gpc2_con = ioremap(0x14CC0080, SZ_4K);
	reg_val = __raw_readl(gpc2_con);
	pr_info("%s(), reg 0x14CC0080 ori value: 0x%08x\n",
		__func__, reg_val);
	#if 0
	reg_val = (reg_val & 0xffffff00) | 0x00000022;
	#else
	reg_val = 0x00222222;
	#endif
	pr_info("%s(), reg 0x14CC0080 new value: 0x%08x\n",
		__func__, reg_val);
...
static void fimc_is_sensor_instanton(struct work_struct *data)
{
	...
	if (device->dtp_check) {
		...
		mod_timer(&device->dtp_timer, jiffies +  msecs_to_jiffies(30000));
		dbg_info("DTP checking...\n");
	}
#endif
	...
	if (instant_cnt) {
		...
		dbg_info("[FRT:D:%d] instant off(fcount : %d, time : %dms)", device->instance,
			device->instant_cnt,
			timetoelapse);
	}
...
static ssize_t show_module(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	pr_info("%s(), will show camera %d 's module info\n",
		__func__, pdev->id);

	mz_project_name = (module_info->project_id == 0x79) ? "M76" : "UNKOWN";
	switch (module_info->vendor_id) {
	case LITEON_ID:
		vendor_name = "LITEON";
		break;
	case SHARP_ID:
	...
	return sprintf(buf, "mz_project_name:%s, vendor_name:%s, "
		"project_id:0x%x, map_version:%s, manufacture_id:0x%x, "
		"module_version:0x%x\n",
		mz_project_name, vendor_name,
		module_info->project_id, module_info->map_version,
...
static DEVICE_ATTR(module,  S_IRUSR | S_IRGRP | S_IROTH, show_module, NULL);
...
static int fimc_is_sensor_probe(struct platform_device *pdev)
{
	...
	device_create_file(&pdev->dev, &dev_attr_module);

#if defined(CONFIG_MEIZU_CAMERA_SPECIAL)
	if (!pdev->id) {
		meizu_special_feature_probe(device);
	}
#endif
p_err:
	...
	struct fimc_is_video_ctx *vctx)
{
	...
	if((fimc_is_eeprom_check_state() == FIMC_IS_EEPROM_STATE_INIT) && (device->pdev->id == SENSOR_POSITION_REAR))
	...
	u32 height)
{
...
#if defined(CONFIG_CAMERA_CUSTOM_SUPPORT)
	device->image.format.field = V4L2_FIELD_ANY;
...
#if defined(CONFIG_CAMERA_CUSTOM_SUPPORT)
	if(module->id == SENSOR_NAME_IMX220) {
		if((device->mode == 6) || (device->mode == 7)) {
	...
	if(module->cfg[device->mode].long_mode) {
		if(device->long_exposure_time)
			device->mode = device->mode + device->long_exposure_time - 1;
	}

	/* can't find proper sensor mode */
	pr_info("%s(), device->mode:%d\n", __func__, device->mode);
	if (device->mode < 0) {
		pr_err("%s(), err!can't find proper sensor mode: %d\n",
			__func__, device->mode);
		ret = -EINVAL;
		goto p_err;
	}
	...
	struct v4l2_streamparm *param)
{
	...
	pr_info("[SEN:D:%d] framerate: req@%dfps, cur@%dfps\n", device->instance,
...
int fimc_is_sensor_s_long_exposure_time(struct fimc_is_device_sensor *device,
	u32 exposure_time)
{
	int ret = 0;
	...
	device->long_exposure_time = exposure_time;
...
int fimc_is_sensor_g_bratio(struct fimc_is_device_sensor *device)
{
	...
	if(device->pdev->id == SENSOR_POSITION_REAR) {
		...
		dbg_info("%s SENSOR_POSITION_REAR \n", __func__);
		...
		dbg_info("%s SENSOR_POSITION_FRONT \n", __func__);
