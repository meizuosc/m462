...
enum scaler_rotation_command {
	...
	TOTALCOUNT_SCALER_ROTATION_COMMAND
};
...
enum scaler_flip_command {
	...
	TOTALCOUNT_SCALER_FLIP_COMMAND
};
...
struct param_isp_captureintent {
    u32 uiCommand;
...
struct is_3AAShotProcessTimeInfo {
    u32 uFrameNumber;

    u32 uSensorStartAbsoluteUs; // CSI frame start INT
    u32 uSensorEndAbsoluteUs; // CSI frame end INT
    u32 uSensorVVI; // CSI frame start INT ~ CSI frame end INT
    u32 uShotInputAbsoluteUs; // Host command shot time
    u32 uFuncCfgAbsoluteUs;// 3AA FuncConfig start time
    u32 uShot2FuncCfg; // Host command shot time ~ 3AA FuncConfig time
    u32 uFuncCfg2ShotDone; // 3AA FuncConfig time ~ ShotDone time
    u32 uShot2ShotDone; // Host command shot time ~ ShotDone time

    u32 uSensorStart2ShotDone; // CSI Frame Start ~ Shot Done
    u32 uSensorEnd2ShotDone; // CSI Frame End ~ Shot Done
};

struct is_ISPShotProcessTimeInfo {
    u32 uFrameNumber;

    u32 uISPStartAbsoluteUs; // ISP frame start INT
    u32 uISPEndAbsoluteUs; // ISP frame end INT
    u32 uISPVVI; // ISP frame start INT ~ ISP frame end INT

    u32 uShotInputAbsoluteUs; // Host command shot time
    u32 uFuncCfgAbsoluteUs;// ISP FuncConfig start time
    u32 uShot2FuncCfg; // Host command shot time ~ ISP FuncConfig time
    u32 uFuncCfg2ShotDone; // ISP FuncConfig time ~ ShotDone time

    u32 uShot2ShotDone; // Host command shot time ~ ShotDone time
    u32 uISPStart2ShotDone; // ISP Frame Start ~ Shot Done
    u32 uISPEnd2ShotDone; // ISP Frame End ~ Shot Done
