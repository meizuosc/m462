	...
	u32 *mbus_code, int index)
{
	...
	for (i = 0; i < ARRAY_SIZE(fimc_is_formats); ++i) {
		...
		if (pixelformat && fmt->pixelformat == *pixelformat) {
			dbg("%s() pixel find:0x%x, %s\n",
				__func__, fmt->pixelformat, fmt->name);
			return fmt;
		}
		if (mbus_code && fmt->mbus_code == *mbus_code) {
			dbg("%s() mbus find:0x%x, %s\n",
				__func__, fmt->pixelformat, fmt->name);
			return fmt;
		}
		if (index == i)
...
p_err:
	info("[VID] %s(), %s(%d) is created\n", __func__, video_name, video_id);
	return ret;
}
