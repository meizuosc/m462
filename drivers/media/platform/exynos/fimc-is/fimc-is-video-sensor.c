...
static int fimc_is_sen_video_open(struct file *file)
{
	...
	mdbgv_sensor("[SS%d:V:%d] %s\n", vctx, video->id, vctx->instance, __func__);
	...
	struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_SENSOR_SET_AE_TARGET:
		...
		break;
	case V4L2_CID_IS_LONG_EXPOSURE_TIME:
		if (fimc_is_sensor_s_long_exposure_time(device, ctrl->value)) {
			err("failed to set long exposure : %d\n - %d",
					ctrl->value, ret);
			ret = -EINVAL;
		}
		break;
