...
p_err:
	mdbgd_group("%s(video_id:%d, otf : %d, ret:%d)\n",
		group, __func__, video_id, otf_input, ret);
	return ret;
}
	...
	struct fimc_is_queue *queue)
{
	...
	if (test_bit(FIMC_IS_GROUP_OTF_INPUT, &group->state)) {
		...
		dbg_info("[OTF] framerate: %d, async shots: %d, shot resource: %d\n",
...
check_completion:
	...
	if (test_bit(FIMC_IS_GROUP_FORCE_STOP, &group->state)) {
		...
		stop_ret = fimc_is_itf_process_stop(device, group_id);
		if (stop_ret) {
	...
	struct fimc_is_frame *ldr_frame)
{
	...
	if ((!pm_qos_request_active(&device->user_qos)) &&
		...
		if (scenario_id > 0) {
			...
			sinfo("GRP:%d dynamic scenario(%d)-[%s]\n",
		...
		if ((scenario_id < 0) && (resourcemgr->dvfs_ctrl.dynamic_ctrl->cur_frame_tick == 0)) {
			...
			sinfo("GRP:%d restore scenario(%d)-[%s]\n",
