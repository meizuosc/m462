...
out:
	...
	if (ret)
		...
		dbg_info("Camera: the %s FW were applied successfully.\n",
...
out:
	...
	if (ret)
		...
		dbg_info("Camera: the %s Setfile were applied successfully.\n",
			((cam_id == CAMERA_SINGLE_REAR) &&
				is_dumped_fw_loading_needed) ? "dumped" : "default");
	...
	struct fimc_is_module_enum *active_sensor)
{
	...
	if (fimc_is_eeprom_check_state() == FIMC_IS_EEPROM_STATE_READONE) {
		...
		dbg_info("Camera : the dumped Cal. data was applied successfully.\n");
	} else if (fimc_is_eeprom_check_state() == FIMC_IS_EEPROM_STATE_INIT) {
		memset((void *)(cal_ptr), 0xFF, FIMC_IS_MAX_CAL_SIZE);
		dbg_info("Camera : did not try to read eeprom. so fill all data with oxff.\n");
	} else {
		memset((void *)(cal_ptr), 0xFF, FIMC_IS_MAX_CAL_SIZE);
		pr_err("%s(), Camera : CRC32 or I2C error , so fill all data with oxff .\n",
			__func__);
		ret = -EIO;
	}
	...
	if (ret)
		warn("%s(), calibration loading is fail", __func__);
	else
		dbg_info("calibration loading is success");
#endif
	...
	u32 group)
{
	...
	if (device->setfile >= ISS_SUB_END) {
		mwarn("%s(), setfile id(%d) is invalid", device, __func__, device->setfile);
...
#if defined(CONFIG_CAMERA_CUSTOM_SUPPORT)//mm78.kim -- Rotate 180 dgree for back camera
      sensor_id = (module_id & 0xFF);
      if(sensor_id == SENSOR_NAME_IMX220)
	  	region->parameter.scalerp.flip.cmd= SCALER_FLIP_COMMAND_XY_MIRROR;
#endif
...
int fimc_is_itf_stream_on(struct fimc_is_device_ischain *device)
{
	...
	if (retry)
		dbg_info("[ISC:D:%d] stream on ready\n", device->instance);
	else
	...
	if ((!pm_qos_request_active(&device->user_qos)) &&
		...
		if (scenario_id >= 0) {
			...
			sinfo("[ISC:D:%d] static scenario(%d)-[%s]\n",
...
int fimc_is_itf_stream_off(struct fimc_is_device_ischain *device)
{
	...
	dbg_info("[ISC:D:%d] stream off ready\n", device->instance);
	...
	u32 input)
{
	...
	dbg_info("%s(%s) : module(%d), flag(%08X), group_id(%d)\n", __func__,
	...
	u32 color_range, u32 *lindex, u32 *hindex, u32 *indexes)
{
	...
	*hindex |= HIGHBIT_OF(PARAM_ISP_OTF_OUTPUT);
	...
	dbg_info("[ISC:D:%d] ISP color range: %s\n", device->instance,
			(crange ? "limited" : "full"));
	...
	*hindex |= HIGHBIT_OF(PARAM_SCALERC_IMAGE_EFFECT);
	...
	dbg_info("[ISC:D:%d] SCC color range: %s\n", device->instance,
			(crange ? "limited" : "full"));
	...
	*hindex |= HIGHBIT_OF(PARAM_SCALERP_IMAGE_EFFECT);
	...
	dbg_info("[ISC:D:%d] SCP color range: %s\n", device->instance,
			(crange ? "limited" : "full"));
	...
	struct fimc_is_queue *queue)
{
	...
	sinfo("[3AA:ISC:%d] 3AA(%d x %d) BDS to SCC(%d x %d) SCC to DIS(%d x %d)\n"
		"\t\t DIS to SCP(%d x %d) SCP to FD(%d x %d)\n",device->instance,
		input_crop[2], input_crop[3],device->chain0_width, device->chain0_height,
		device->chain1_width, device->chain1_height,device->chain2_width, device->chain2_height,
