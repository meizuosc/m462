...
inline static void fimc_is_set_qos_init(struct fimc_is_core *core, bool on)
{
	...
	if (on) {
		...
		pr_debug("[RSC] %s: QoS LOCK [INT(%d), MIF(%d), CAM(%d), DISP(%d) CPU(%d/%d)]\n",
...
int fimc_is_runtime_resume(struct device *dev)
{
	...
	pm_stay_awake(dev);
	dbg_info("FIMC_IS runtime resume in\n");
	...
	dbg_info("FIMC-IS runtime resume out\n");
