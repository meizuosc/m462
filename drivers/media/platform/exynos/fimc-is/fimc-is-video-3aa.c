...
static int fimc_is_3aa_video_open(struct file *file)
{
	...
	mdbgv_3aa("[3A%d:V:%d] %s\n", vctx, GET_3AA_ID(video), vctx->instance, __func__);
	...
	mdbgv_3aa("[3A%d:V:%d] <-> [ISP:V:%d] \n", vctx, GET_3AA_ID(video),
	...
	struct v4l2_format *format)
{
	...
	mdbgv_3aa("%s, %s device\n", vctx, __func__,
		V4L2_TYPE_IS_OUTPUT(format->type) ? "OUTPUT": "CAPTURE");
	...
	unsigned int count)
{
	...
	mdbgv_3aa("%s, %s device\n", vctx, __func__,
		V4L2_TYPE_IS_OUTPUT(vbq->type) ? "OUTPUT" : "CAPTURE");
