...
#define MUXQ_STATUS_XMIT_FULL (1<<0)
#define MUXQ_STATUS_XMIT_QUEUED (1<<1)

	u32 status;
	spinlock_t lock;
	u32 length;
	u32 stat_tx_drop;
	u32 stat_tx_delt_bytes;
	u32 stat_rx_delt_bytes;
	u32 stat_tx_delt_time;
};
...
typedef struct
{
	...
	u32 length;
}__attribute__ ((packed))ts0710_backup_header;
...
#ifdef TS0710DEBUG
#define TS0710_DEBUG(fmt, arg...) printk(KERN_DEBUG "TS07.10: " fmt , ## arg)
#else
