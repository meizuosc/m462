...
static void s3c24xx_serial_resetfifo(struct uart_port *port);
static void s3c24xx_serial_pm(struct uart_port *port, unsigned int level,
			      unsigned int old);
static struct uart_driver s3c24xx_uart_drv;

#ifdef MZ_UART_DEBUG
static unsigned int rx_cnt_debug;
static unsigned int tx_cnt_debug;
void uart_dump_reg(struct uart_port *uport)
{
	unsigned int i = 0;
	unsigned int cfg = 0;

	dev_info(uport->dev, "%s(), for test, dump registers\n",
		__func__);
	for (i = 0x0; i <= 0x0038; i += 0x04) {
		cfg = rd_regl(uport, i);
		dev_info(uport->dev, "reg 0x%04x = 0x%08x\n",
			i, cfg);
	}
}
#endif
...
static void s3c64xx_serial_lpm(struct work_struct *work)
{
	struct s3c24xx_uart_port *ourport =
		container_of(work, struct s3c24xx_uart_port,
						uart_port_lpm_work);
	struct uart_port *port = &ourport->port;

	if (ourport->uart_lpm_wake)
		uart_resume_port(&s3c24xx_uart_drv, port);
	else
		uart_suspend_port(&s3c24xx_uart_drv, port);
}

static int s3c24xx_serial_set_wake(struct uart_port *port, unsigned int state)
{
	struct s3c24xx_uart_port *ourport =
	...
	if (lpass_subip_uart) {
		...
		ourport->uart_lpm_wake = state;

		if (!ourport->aud_uart_notifier_suspend)
			schedule_work(&ourport->uart_port_lpm_work);
	}
...
static void s3c24xx_serial_start_tx(struct uart_port *port)
{
	...
	if (s3c24xx_serial_lpm_suspend(ourport)) {
		/* wakeup serial port */
		s3c24xx_serial_set_wake(port, 1);
		return;
	}
...
rx_use_cpu:
	...
	while (max_count-- > 0) {
		...
		if (unlikely(uerstat & S3C2410_UERSTAT_ANY)) {
			dev_err(port->dev, "rxerr: port ch=0x%02x, rxs=0x%08x\n",
			    ch, uerstat);
			...
			if (uerstat & S3C2410_UERSTAT_BREAK) {
				...
				if (uart_handle_break(port)) {
					dev_info(port->dev, "!!break, ignore it\n");
					goto ignore_char;
				}
			}
		...
		if (uart_handle_sysrq_char(port, ch)) {
			dev_info(port->dev, "!!sysrq char, ignore it\n");
			goto ignore_char;
		}

		#ifdef MZ_UART_DEBUG
		if (BT_UART_NUMBER == ourport->pdev->id &&
			rx_cnt_debug < 64) {
			pr_info("rx[%d] = 0x%x, uerstat:0x%x, flag:0x%x\n",
				rx_cnt_debug, ch, uerstat, flag);
			rx_cnt_debug++;
		}
		#endif
		uart_insert_char(port, uerstat, S3C2410_UERSTAT_OVERRUN,
...
static irqreturn_t s3c24xx_serial_tx_chars(int irq, void *id)
{
	...
	if (port->x_char) {
		#ifdef MZ_UART_DEBUG
		if (BT_UART_NUMBER == ourport->pdev->id) {
			pr_info("x_char, tx[%d] = 0x%x\n",
				tx_cnt_debug++, port->x_char);
		}
		#endif
...
tx_use_cpu:
	...
	while (!uart_circ_empty(xmit) && count-- > 0) {
		...
		if (BT_UART_NUMBER == ourport->pdev->id &&
			tx_cnt_debug < 64) {
			pr_info("tx[%d] = 0x%x\n",
				tx_cnt_debug, xmit->buf[xmit->tail]);
			tx_cnt_debug++;
		}
		#endif
...
static void s3c64xx_serial_qos_func(struct work_struct *work)
{
	...
	if (pm_qos_request_active(&ourport->s3c24xx_uart_mif_qos))
		pm_qos_update_request_timeout(&ourport->s3c24xx_uart_mif_qos,
				ourport->mif_qos_val, ourport->qos_timeout);

	if (pm_qos_request_active(&ourport->s3c24xx_uart_cpu_qos))
...
void aud_uart_gpio_idle(struct device *dev)
{
	...
	aud_uart_gpio_cfg(dev, S3C24XX_UART_PORT_LPM);
			      ...
			      unsigned int old)
{
	...
	dev_info(port->dev, "%s(), port:0x%p, level:%d, old:%d\n",
		__func__, port, level, old);

	switch (level) {
	case S3C24XX_UART_PORT_SUSPEND:
		if (!(atomic_read(&ourport->serial_suspend) || ourport->port.suspended)) {
			/* disable auto flow control & set nRTS for High */
			umcon = rd_regl(port, S3C2410_UMCON);
			umcon &= ~(S3C2410_UMCOM_AFC | S3C2410_UMCOM_RTS_LOW);
			wr_regl(port, S3C2410_UMCON, umcon);
			dev_info(port->dev, "%s(), not suspend previously\n", __func__);
		} else {
			dev_warn(port->dev, "%s(), already suspend previously\n", __func__);
		}
#ifdef CONFIG_PM_RUNTIME
		if (of_find_property(ourport->pdev->dev.of_node,
			...
			if (!atomic_read(&ourport->serial_suspend)) {
				...
				dev_warn(port->dev, "%s(), suspend already\n", __func__);
			}
		} else {
			clk_disable_unprepare(ourport->clk);
		}
				       ...
				       struct ktermios *old)
{
	...
	printk("uart: ulcon = 0x%08x, ucon = 0x%08x, ufcon = 0x%08x, "
		"umcon = 0x%08x\n",
				   ...
				   struct s3c2410_uartcfg *cfg)
{
	...
	wr_regl(port, S3C2410_UFCON, cfg->ufcon |
		S3C2410_UFCON_RESETTX | S3C2410_UFCON_RESETRX);
	udelay(10);
	wr_regl(port, S3C2410_UFCON, cfg->ufcon);
...
static void s3c24xx_serial_resetfifo(struct uart_port *port)
{
	...
	value = rd_regl(port, S3C2410_UFCON);
	/* reset both fifos */
	wr_regl(port, S3C2410_UFCON, value |
		S3C2410_UFCON_RESETTX | S3C2410_UFCON_RESETRX);
	udelay(10);
	wr_regl(port, S3C2410_UFCON, value);
...
#ifdef MZ_UART_DEBUG
static ssize_t s3c24xx_serial_show_reg(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct uart_port *port = s3c24xx_dev_to_port(dev);
...
static DEVICE_ATTR(reg, S_IRUGO, s3c24xx_serial_show_reg, NULL);
#endif
					...
					unsigned long event, void *v)
{
	...
	case PM_SUSPEND_PREPARE:
		cancel_work_sync(&ourport->uart_port_lpm_work);
		uart_suspend_port(&s3c24xx_uart_drv, port);
...
int bt_init_uart(int enable)
{
	struct uart_port *bt_uart_port;
	struct uart_state *state;
	unsigned int pm_state;
	unsigned int pm_state_old;

	bt_uart_port= &(s3c24xx_serial_ports[BT_UART_NUMBER].port);
	if (IS_ERR_OR_NULL(bt_uart_port)) {
		pr_err("%s(), uart_port is not ready:0x%p\n",
			__func__, bt_uart_port);
		return -EINVAL;
	}

	state = bt_uart_port->state;
	if (IS_ERR_OR_NULL(state)) {
		pr_err("%s(), uart_state is not ready:0x%p\n",
			__func__, state);
		return -EINVAL;
	}

	pr_debug("%s(), state's pm_state is %d\n",
		__func__, state->pm_state);
	if (UART_PM_STATE_ON == state->pm_state) {
		pr_info("%s(), AUD UART has already been enabled\n",
			__func__);
		return 0;
	}

	if (enable) {
		pm_state = UART_PM_STATE_ON;
		pm_state_old = state->pm_state;
	} else {
		pm_state = state->pm_state;
		pm_state_old = UART_PM_STATE_ON;
	}

	pr_debug("%s(), for enable:%d, set pm_state:%d, pm_state_old:%d\n",
		__func__, enable, pm_state, pm_state_old);

	s3c24xx_serial_pm(bt_uart_port, pm_state,
		pm_state_old);
...
static int s3c24xx_serial_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		pdev->id = probe_index = ret;
	} else {
		probe_index = pdev->id;
	}

	pr_debug("s3c24xx_serial_probe(%p) %d\n", pdev, probe_index);

#if !defined(CONFIG_RECOVERY_KERNEL) && !defined(CONFIG_CHARGE_KERNEL)
	if (probe_index == BT_UART_NUMBER) {
		dev_info(&pdev->dev, "%s(), set wake_peer for AUD-UART\n",
			__func__);
		s3c2410_serial_wake_peer[probe_index] = bt_uart_wake_peer;
	}
#endif
	...
	if (ret < 0)
		dev_err(&pdev->dev, "failed to add reg attr.\n");
#endif
