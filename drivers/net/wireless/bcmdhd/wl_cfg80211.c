...
static void wl_scan_prep(struct wl_scan_params *params, struct cfg80211_scan_request *request)
{
	...
	params->scan_type = 0;
	params->nprobes = -1;
	#ifndef MEIZU_FAST_SCAN
	params->active_time = -1;
	#else
	if (unlikely(0 == escan_cnt++)) {
		params->active_time = 10;
	} else {
		params->active_time = -1;
	}
	pr_debug("%s(), active_time=%d\n", __func__, params->active_time);
	#endif
	...
	const wl_event_msg_t *e, void *data)
{
	...
	get_monotonic_boottime(&ts_new);
	if (ts_new.tv_sec > (ts.tv_sec + 1)) {
		mic_cnt = 0;
		ts.tv_sec = ts_new.tv_sec;
	} else {
		mic_cnt++;
	}
	if (mic_cnt < 4) {
		printk("mic_cnt = %d, skip send \n", mic_cnt);
		return 0;
	}
	#endif
	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		if (wl_get_drv_status_all(wl, FINDING_COMMON_CHANNEL)) {
			...
			if (bi_length > ESCAN_BUF_SIZE - list->buflen) {
				WL_ERR(("Compared to bi_lenght 0x%x, Buffer size 0x%x is too small: ignoring\n",
					bi_length, ESCAN_BUF_SIZE - list->buflen));
				goto exit;
			}
	...
	while (down_interruptible (&tsk->sema) == 0) {
		...
		while ((e = wl_deq_event(wl))) {
			...
			if ((wl_is_p2p_event(e) == TRUE) && (wl->p2p_wdev)) {
				...
				struct net_device *ndev = NULL;
				ndev = dhd_idx2net((struct dhd_pub *)(wl->pub), e->emsg.ifidx);
				if (ndev)
