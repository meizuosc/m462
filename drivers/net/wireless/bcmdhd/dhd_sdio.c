...
#define HT_AVAIL_ERROR_MAX 10
	...
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
	pr_info("%s, bus->alp_only:%d, on:%d, pendok:%d\n",
		__func__, bus->alp_only, on, pendok);
	...
	if (!KSO_ENAB(bus)) {
		pr_info("%s(), kso not enable yet, return directly\n", __func__);
		return BCME_OK;
	}

	if (SLPAUTO_ENAB(bus)) {
		bus->clkstate = (on ? CLK_AVAIL : CLK_SDONLY);
		pr_info("%s(), SLPAUTO_ENAB() is true, set clkstate to %d and return\n",
			__func__, bus->clkstate);
		return BCME_OK;
	}

	#if 1
	bus->clkstate = (on ? CLK_AVAIL : CLK_SDONLY);
	pr_info("%s(), !!!directly return here, as we will use KSO to enable and disable bus clk\n",
		__func__);
	return BCME_OK;
	#endif
	...
	if (on) {
		...
		pr_info("%s(), clkctl:%d, err:%d, bus->alp_only:%d\n",
			__func__, clkctl, err, bus->alp_only);
		...
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only)) {
			DHD_ERROR(("%s: HT Avail timeout (%d): clkctl 0x%02x, alp_only:%d\n",
			           __FUNCTION__, PMU_MAX_TRANSITION_DLY, clkctl, bus->alp_only));
			return BCME_ERROR;
		}
