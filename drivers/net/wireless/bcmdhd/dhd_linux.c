...
dhd_open(struct net_device *net)
{
	...
	#ifdef MEIZU_FAST_SCAN
	pr_debug("%s(), modify escan_cnt to 0\n", __func__);
	escan_cnt = 0;
	#endif
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
...
#ifdef ROAM_ENABLE
	uint roamvar = 0;
	int roam_trigger_a[2] = {CUSTOM_ROAM_TRIGGER_SETTING_A, WLC_BAND_5G};
	int roam_trigger_b[2] = {CUSTOM_ROAM_TRIGGER_SETTING_B, WLC_BAND_2G};
	...
	if (!op_mode && (strstr(fw_path, "_apsta") == NULL)) {
		int mchan_algo_val = 1;
		int mchan_bw_val = 30;
		bcm_mkiovar("mchan_algo", (char *)&mchan_algo_val, 4, iovbuf, sizeof(iovbuf));
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
			sizeof(iovbuf), TRUE, 0)) < 0) {
			DHD_ERROR(("%s mchan_algo_val for wifi failed  %d\n", __FUNCTION__, ret));
		}

		bcm_mkiovar("mchan_bw", (char *)&mchan_bw_val, 4, iovbuf, sizeof(iovbuf));
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
			sizeof(iovbuf), TRUE, 0)) < 0) {
			DHD_ERROR(("%s mchan_bw_val for wifi failed  %d\n", __FUNCTION__, ret));
		}
		printk("modify mchan in sta mode \n");
	}
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_TRIGGER, roam_trigger_b,
		sizeof(roam_trigger_b), TRUE, 0)) < 0)
		DHD_ERROR(("%s: roam trigger b set failed %d\n", __FUNCTION__, ret));

	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_TRIGGER, roam_trigger_a,
		sizeof(roam_trigger_a), TRUE, 0)) < 0)
		DHD_ERROR(("%s: roam trigger a set failed %d\n", __FUNCTION__, ret));
	...
	//extend pm2_sleep_ret to avoid frequently entering PM mode in very low traffic
	bcm_mkiovar("pm2_sleep_ret", (char *)&pm2_sleep_ret, 4, iovbuf, sizeof(iovbuf));
	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
	...
	dhd->pktfilter[DHD_UNICAST_FILTER_NUM] = "100 0 0 0 0x01 0x01";
	dhd->pktfilter[DHD_BROADCAST_FILTER_NUM] = NULL;
	dhd->pktfilter[DHD_MULTICAST4_FILTER_NUM] = NULL;
	dhd->pktfilter[DHD_MULTICAST6_FILTER_NUM] = NULL;
	/* Add filter to pass multicastDNS packet and NOT filter out as Broadcast */
	dhd->pktfilter[DHD_MDNS_FILTER_NUM] = "104 0 0 23 0xff000000000000ffffffff0000ffff 0x11000000000000effffffa0000076c";
