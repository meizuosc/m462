...
wl_cfgp2p_add_p2p_disc_if(void)
{
	...
	if (IS_ERR_OR_NULL(wl) || IS_ERR_OR_NULL(wl->p2p)) {
		pr_err("%s(), check IS_ERR_OR_NULL(), wl: %ld, "
			"wl->p2p: %ld\n", __func__,
			IS_ERR_OR_NULL(wl), IS_ERR_OR_NULL(wl->p2p));
		BUG();
	...
	if (wl->p2p_wdev) {
		CFGP2P_ERR(("err! p2p_wdev defined already. will try to del it!\n"));
		if (wl_cfgp2p_del_p2p_disc_if(wl->p2p_wdev)) {
			CFGP2P_ERR(("err! failed to del p2p_wdev!\n"));
