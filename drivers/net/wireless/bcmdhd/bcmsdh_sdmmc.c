                     ...
                     uint addr, void *pkt)
{
	...
	if (0 != dma_len) {
#ifdef M76_RXCHAIN_PATCH
		unsigned int nents,i,left_size;
		struct scatterlist  *sg_ptr;
		unsigned int seg_size = 4096;
#endif
...
#ifdef M76_RXCHAIN_PATCH
			nents = (pkt_len - 1) / seg_size + 1;
			if (nents > 1) {
				uint8 *buf = (uint8*)PKTDATA(sd->osh, pnext) ;
				left_size = pkt_len;
				for_each_sg(&sd->sg_list[SGCount], sg_ptr, nents, i) {
					sg_set_page(sg_ptr, virt_to_page(buf + (i * seg_size)),
					min(seg_size, left_size),
					offset_in_page(buf + (i * seg_size)));
					left_size = left_size - seg_size;
				}
				SGCount = SGCount + nents;
			}
			else
#endif
