...
static struct gendisk *emmc_disk = NULL;
...
dev_t emmc_lookup_partition(const char *part_name, sector_t *start, sector_t *nr_sect)
{
...
int emmc_partition_rw(const char *part_name, int write, loff_t offset,
						void *buffer, size_t len)
{
	...
	if (!devt) {
		pr_err("%s: can't find eMMC partition(%s)\n", __func__, part_name);
		return -ENODEV;
	}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	/* Forbid setting MMC block device's r/w attribute */
	if (cmd == BLKROSET) {
		ret = -EACCES;
		return ret;
	}
...
static int __maybe_unused mmc_blk_alloc_parts(struct mmc_card *card, struct mmc_blk_data *md)
{
	int idx, ret = 0;
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	if (!strcmp(md->disk->disk_name, "mmcblk0")) {
		...
		/* set /dev/block/mmcblk0 Read-only */
		emmc_disk->part0.policy = 1;

		disk_part_iter_init(&piter, emmc_disk, DISK_PITER_INCL_EMPTY);
		while ((part = disk_part_iter_next(&piter))) {
			if (!strcmp(part->info->volname, "cache") ||
			...
			if (!strcmp(part->info->volname, "modem")) {
					continue;
			}
#endif
			part->policy = 1;
