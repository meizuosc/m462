...
static void exynos_sfr_save(unsigned int i)
{
	...
	if (host->pdata->enable_cclk_on_suspend) {
		...
		if(dw_mci_ciu_clk_en(host, false))
			dev_dbg(host->dev, "ciu_clk_enable_fail!\n");
	}
...
static int dw_mci_exynos_execute_tuning(struct dw_mci *host, u32 opcode)
{
	...
	unsigned int drv_str_retries = MAX_TUNING_RETRIES;
	...
	dev_err(host->dev, "sample_good: 0x %02x best_sample: 0x %02x tuning_loop %x \n",
		sample_good, best_sample,tuning_loop);
...
static int dw_mci_exynos_set_def_caps(struct dw_mci *host)
{
	...
	case 1:
		...
		pr_info("%s(), set def caps 0x%x for MMC1\n", __func__, ret);
		break;
	case 2:
...
static void dw_mci_exynos_shutdown(struct device *dev)
{
