...
struct platform_device *pdev_wl_dwmci = NULL;
...
#define TP_MONITOR_SEC_DIV		2
#define TP_MONITOR_JIFFIES		(HZ / TP_MONITOR_SEC_DIV)
#define TP_MONITOR_PM_QOS_LOCK_US	(1000 * 1000)
#define TP_MONITOR_PM_QOS_LOCK_MS	(TP_MONITOR_PM_QOS_LOCK_US / 1000)

static void suspend_tp_monitor(struct dw_mci *host)
{
	host->suspended = 1;
	if (host->pdata->tp_mon_tbl) {
		if (delayed_work_pending(&host->tp_mon))
			cancel_delayed_work(&host->tp_mon);
		host->transferred_cnt = 0;
	}
}

static void resume_tp_monitor(struct dw_mci *host)
{
	if (host->pdata->tp_mon_tbl)
		host->transferred_cnt = 0;
	host->suspended = 0;
		...
		struct device_attribute *attr, char *buf)
{
	...
	/*
	 * print cmd_log from prev. 24 to last
	 * guess Max buf size is about 1KB
	 */
	 if (host->debug_info->en_logging & DW_MCI_DEBUG_ON_CMD) {
		 ...
		 total_len += snprintf(buf + (sizeof(char)*6), PAGE_SIZE, "HOST%1d\n", mmc->index);
		...
		bool new_cmd, u8 flags, u32 rintsts)
{
	...
	unsigned int count;
	struct dw_mci_cmd_log *cmd_log = host->debug_info->cmd_log;
		...
		enum dw_mci_req_log_state log_state, enum dw_mci_state state)
{
	int cpu = raw_smp_processor_id();
	unsigned int count;
	struct dw_mci_req_log *req_log = host->debug_info->req_log;

	if (!(host->debug_info->en_logging & DW_MCI_DEBUG_ON_REQ))
		return;
				   ...
				   struct mmc_command *cmd)
{
	...
	if (data) {
		...
		host->rw_flags = data->flags;
		if (host->pdata->tp_mon_tbl && !host->suspended) {
			host->transferred_cnt += data->blksz * data->blocks;
			schedule_delayed_work(&host->tp_mon, TP_MONITOR_JIFFIES);
		}
	}
...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		...
		if (slot->host->pdata->tp_mon_tbl)
			schedule_delayed_work(&slot->host->tp_mon, TP_MONITOR_JIFFIES);
		break;
	case MMC_POWER_OFF:
		cclk_request_turn_off = 1;
		...
		if (slot->host->pdata->tp_mon_tbl) {
			...
			pm_qos_update_request(&slot->host->pm_qos_cpu_num, 0);
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
			pm_qos_update_request(&slot->host->pm_qos_kfc, 0);
			pm_qos_update_request(&slot->host->pm_qos_kfc_num, 0);
#endif
		}
		break;
	default:
...
static void dw_mci_tp_mon(struct work_struct *work)
{
	...
	s32 kfc_num_value = 0;
#endif
	static int prev_range[2];
	static unsigned long prev_jiffies[2];
	int qos_request = 1;
	int hid = host->pdev->id;
	int hmp_boost = 0;
	...
	host->transferred_cnt *= TP_MONITOR_SEC_DIV;

	while (tp_tbl->range) {
		if ((host->transferred_cnt > tp_tbl->range) && (flags & tp_tbl->rw_flags)) {
			pr_debug("%s: flags = %s size = %d >= range = %d\n", __func__,
				(flags & MMC_DATA_READ) ? "read" : "write", host->transferred_cnt, tp_tbl->range);
			break;
		}
		tp_tbl++;
	}
...
#if 0
	if (hid == 0) {
		if (((host->transferred_cnt >= MMC_READ_SPEEDUP_SIZE) && (flags & MMC_DATA_READ))
			|| ((host->transferred_cnt >= MMC_WRITE_SPEEDUP_SIZE) && (flags & MMC_DATA_WRITE))) {
			hmp_boost = 1;
			pr_debug("%s: force migrate the emmc operation to big cpu\n", __func__);
		}
	}
#endif
	...
	cpu_num_value = tp_tbl->cpu_num_value;
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
	kfc_lock_value = tp_tbl->kfc_lock_value;
	kfc_num_value = tp_tbl->kfc_num_value;
#endif

#ifndef CONFIG_ARM_EXYNOS_MP_CPUFREQ
	dev_dbg(host->dev, "prepare: %d byte/s cnt=%d mif=%d cpu=%d\n",
						host->transferred_cnt,
						host->cmd_cnt,
						mif_lock_value,
						cpu_lock_value);
#else
	dev_dbg(host->dev, "prepare: %d byte/s cnt=%d mif=%d cpu=%d kfc=%d\n",
						host->transferred_cnt,
						host->cmd_cnt,
						mif_lock_value,
						cpu_lock_value,
	...
	if (tp_tbl->range == 0)
		...
		if (hid && prev_range[hid] >= tp_tbl->range) {
			if (jiffies_to_msecs(jiffies - prev_jiffies[hid]) <
					TP_MONITOR_PM_QOS_LOCK_MS)
				qos_request = 0;
		}
	}
	if (qos_request) {
#ifndef CONFIG_ARM_EXYNOS_MP_CPUFREQ
		dev_dbg(host->dev, "request: %d byte/s mif=%d cpu=%d\n",
						host->transferred_cnt,
						mif_lock_value,
						cpu_lock_value);
#else
		dev_dbg(host->dev, "request: %d byte/s mif=%d cpu=%d kfc=%d\n",
                                                host->transferred_cnt,
                                                mif_lock_value,
                                                cpu_lock_value,
                                                kfc_lock_value);
		pm_qos_update_request_timeout(&host->pm_qos_kfc,
						kfc_lock_value,
						TP_MONITOR_PM_QOS_LOCK_US << hid);
		pm_qos_update_request_timeout(&host->pm_qos_kfc_num,
						kfc_num_value,
						TP_MONITOR_PM_QOS_LOCK_US << hid);
#endif
		...
		pm_qos_update_request_timeout(&host->pm_qos_cpu,
						cpu_lock_value,
						TP_MONITOR_PM_QOS_LOCK_US << hid);
		pm_qos_update_request_timeout(&host->pm_qos_cpu_num,
						cpu_num_value,
						TP_MONITOR_PM_QOS_LOCK_US << hid);
		if (hmp_boost)
			set_hmp_boostpulse(TP_MONITOR_PM_QOS_LOCK_US << hid);

		prev_range[hid] = tp_tbl->range;
		prev_jiffies[hid] = jiffies;
	}
...
static int dw_mci_init_slot(struct dw_mci *host, unsigned int id)
{
	...
	dev_info(host->dev, "%s, mmc's caps:0x%x, caps2:0x%x; "
		"cd_type:%d\n", __func__,
		mmc->caps, mmc->caps2,
		host->pdata->cd_type);
	if (host->pdata->cd_type == DW_MCI_CD_EXTERNAL) {
		pr_info("%s(), call ext_cd_init\n", __func__);
...
static struct dw_mci_board *dw_mci_parse_dt(struct dw_mci *host)
{
	...
	if (of_find_property(np, "ignore-pm-notify", NULL))
	...
	id = of_alias_get_id(dev->of_node, "mshc");
	host->pdev->id = id;
	pr_info("%s(), mmc id is %d\n", __func__, id);
	if (id == WL_MMC_NUM) {
		pr_info("%s(), setup dwmmc1(wlan) specific\n", __func__);
		pdev_wl_dwmci = to_platform_device(dev);
		pdata->get_bus_wd = exynos_dwmci1_get_bus_wd;
		if (DW_MCI_CD_EXTERNAL == pdata->cd_type) {
			pr_info("%s(), cd type is external\n", __func__);
			pdata->ext_cd_init = ext_cd_init_dwmci1;
			pdata->ext_cd_cleanup = ext_cd_cleanup_dwmci1;
		}
		pdata->tp_mon_tbl = exynos_dwmci_tp_mon1_tbl;
	}

	if (id == FLASH_MMC_NUM) {
		pr_info("%s(), mmc id is %d\n", __func__, id);
		pdata->tp_mon_tbl = exynos_dwmci_tp_mon0_tbl;
	}
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	if (ret) {
		...
		pr_info("%s(), host->bus_hz:%u\n", __func__, host->bus_hz);
	}
	...
	dev_info(host->dev, "%s(), pdata->tp_mon_tbl:%p\n",
		__func__, host->pdata->tp_mon_tbl);
	if (host->pdata->tp_mon_tbl) {
		host->transferred_cnt = 0;
		...
		pm_qos_add_request(&host->pm_qos_cpu_num,
					PM_QOS_CPU_NUM_MIN, 0);
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
		pm_qos_add_request(&host->pm_qos_kfc,
					PM_QOS_KFC_FREQ_MIN, 0);
		pm_qos_add_request(&host->pm_qos_kfc_num,
					PM_QOS_KFC_NUM_MIN, 0);
#endif
	}
...
err_workqueue:
	...
	if (host->pdata->tp_mon_tbl) {
		...
		pm_qos_remove_request(&host->pm_qos_cpu_num);
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
		pm_qos_remove_request(&host->pm_qos_kfc);
		pm_qos_remove_request(&host->pm_qos_kfc_num);
#endif
	}
...
int dw_mci_suspend(struct dw_mci *host)
{
	...
	if (host->pdata->tp_mon_tbl &&
		...
		pm_qos_update_request(&host->pm_qos_cpu_num, 0);
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
		pm_qos_update_request(&host->pm_qos_kfc, 0);
		pm_qos_update_request(&host->pm_qos_kfc_num, 0);
#endif
		host->transferred_cnt = 0;
		host->cmd_cnt = 0;
	}

	suspend_tp_monitor(host);
