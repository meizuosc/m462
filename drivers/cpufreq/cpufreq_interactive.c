...
static DEFINE_RAW_SPINLOCK(cpufreq_limit_lock);
static u64 cpufreq_limitpulse_endtime;

#define get_cpufreq_limit_freq(cpu)   (((cpu) < NR_CA7) ? 1500000 : 1200000)

static inline int cpufreq_limit(u64 now)
{
	int ret;
	unsigned long flags;

	raw_spin_lock_irqsave(&cpufreq_limit_lock, flags);
	if (now < cpufreq_limitpulse_endtime)
		ret = 1;
	else
		ret = 0;
	raw_spin_unlock_irqrestore(&cpufreq_limit_lock, flags);

	return ret;
}


static inline int get_cpufreq_limit(u64 now)
{
	return get_hmp_boost() || cpufreq_limit(now);
}

int set_cpufreq_limitpulse(int duration)
{
	unsigned long flags;
	u64 limitpulse_endtime;

	if (duration < 0)
		return -EINVAL;

	limitpulse_endtime = ktime_to_us(ktime_get()) + duration;

	raw_spin_lock_irqsave(&cpufreq_limit_lock, flags);
	if (limitpulse_endtime > cpufreq_limitpulse_endtime)
		cpufreq_limitpulse_endtime = limitpulse_endtime;
	raw_spin_unlock_irqrestore(&cpufreq_limit_lock, flags);
...
#ifdef CONFIG_PRE_HISPEED
	/* Intermediate frequency from lo speed to hispeed. (default max) */
	unsigned int prehispeed_freq;
#endif // CONFIG_PRE_HISPEED
...
#ifdef CONFIG_DOWN_DELAY
	/*
	 * Wait this long before decreasing speed , by default a
	 * single min_sample_time.
	 */
	spinlock_t down_delay_lock;
	unsigned int *down_delay;
	int ndown_delay;
#endif // CONFIG_DOWN_DELAY
	/*
...
#define DEFAULT_MIN_SAMPLE_TIME (80 * USEC_PER_MSEC)
#ifdef CONFIG_DOWN_DELAY
static unsigned int default_down_delay[] = {
...
#ifdef CONFIG_DOWN_DELAY
static unsigned int freq_to_down_delay(
	struct cpufreq_interactive_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	...
	ret = tunables->min_sample_time;

	spin_lock_irqsave(&tunables->down_delay_lock, flags);

	for (i = 0 ; i < tunables->ndown_delay - 1 &&
			 freq <= tunables->down_delay[i+1]; i+=2)
	;

	ret = tunables->down_delay[i];

	spin_unlock_irqrestore(&tunables->down_delay_lock, flags);
	return ret;
}
#endif // CONFIG_DOWN_DELAY
		...
		unsigned int loadadjfreq)
{
	...
	do {
		...
		if (freq > prevfreq) {		// CPU Ratio > TargetLoads
		...
		} else if (freq < prevfreq) { // CPU Ratio < TargetLoads
...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	struct cpufreq_interactive_tunables *tunables =
		pcpu->policy->governor_data;
	unsigned int new_freq, limit_freq;
	unsigned int loadadjfreq;
	unsigned int index;
	unsigned long flags;
	bool boosted;
...
#ifdef CONFIG_PRE_HISPEED
		if (pcpu->target_freq < tunables->prehispeed_freq) {
			new_freq = tunables->prehispeed_freq;
		} else
#endif // CONFIG_PRE_HISPEED
...
#ifdef CONFIG_PRE_HISPEED
			if(!boosted && new_freq < tunables->prehispeed_freq)
				new_freq = tunables->prehispeed_freq;
			else
#endif // CONFIG_PRE_HISPEED
	...
	/* Limit the minimal cpu freq for HMP boost */
	if (get_cpufreq_limit(now)) {
		limit_freq = get_cpufreq_limit_freq(data);
		if (new_freq < limit_freq)
			new_freq = limit_freq;
	}

	/* FIXME : Discrete DVFS level
	* In some cases, CPU ratio does not reflect the next frequency
	*				prediction. Cuz the hispeed freq
	*/
	 ...
	 * floor frequency for the freq_down_delay since last validated.
	 */

	freq_down_delay = freq_to_down_delay(tunables, pcpu->floor_freq);
...
#ifdef CONFIG_DOWN_DELAY
static ssize_t show_down_delay(
	struct cpufreq_interactive_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
...
static ssize_t store_down_delay(
	struct cpufreq_interactive_tunables *tunables,
	const char *buf, size_t count)
{
	int ntokens;
	unsigned int *new_down_delay = NULL;
	unsigned long flags;

	new_down_delay = get_tokenized_data(buf, &ntokens);
	if (IS_ERR(new_down_delay))
		return PTR_RET(new_down_delay);

	spin_lock_irqsave(&tunables->down_delay_lock, flags);
	if (tunables->down_delay != default_down_delay)
		kfree(tunables->down_delay);
	tunables->down_delay = new_down_delay;
	tunables->ndown_delay = ntokens;
	spin_unlock_irqrestore(&tunables->down_delay_lock, flags);
	return count;
...
#endif
#ifdef CONFIG_PRE_HISPEED
	if (tunables->prehispeed_freq > tunables->hispeed_freq)
		tunables->prehispeed_freq = tunables->hispeed_freq;
#endif
	return count;
}

#ifdef CONFIG_PRE_HISPEED
static ssize_t show_prehispeed_freq(struct cpufreq_interactive_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->prehispeed_freq);
}

static ssize_t store_prehispeed_freq(struct cpufreq_interactive_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
	...
	if(val <= tunables->hispeed_freq)
		tunables->prehispeed_freq = val;
	else
		tunables->prehispeed_freq = tunables->hispeed_freq;
...
#ifdef CONFIG_DOWN_DELAY
show_store_gov_pol_sys(down_delay);
#endif
show_store_gov_pol_sys(hispeed_freq);
#ifdef CONFIG_PRE_HISPEED
show_store_gov_pol_sys(prehispeed_freq);
#endif
...
#ifdef CONFIG_DOWN_DELAY
gov_sys_pol_attr_rw(down_delay);
#endif
gov_sys_pol_attr_rw(hispeed_freq);
#ifdef CONFIG_PRE_HISPEED
gov_sys_pol_attr_rw(prehispeed_freq);
#endif
		...
		unsigned int event)
{
	...
	pr_debug("%s: event = %s, policy->cpu = %d start\n", __func__, event_str[event], policy->cpu);
	...
	case CPUFREQ_GOV_POLICY_INIT:
		if (have_governor_per_policy()) {
			...
			pr_debug("%s: event = %s, policy->cpu = %d end 1\n", __func__, event_str[event], policy->cpu);
			return 0;
		}
		...
		if (!tunables) {
			...
			pr_debug("%s: event = %s, policy->cpu = %d end 2\n", __func__, event_str[event], policy->cpu);
			return -ENOMEM;
		}
		...
		if (!tuned_parameters[policy->cpu]) {
			...
			tunables->ndown_delay =
				ARRAY_SIZE(default_down_delay);
#endif
...
#endif
#ifdef CONFIG_DOWN_DELAY
		spin_lock_init(&tunables->down_delay_lock);
#endif
		policy->governor_data = tunables;
		if (!have_governor_per_policy())
	...
	case CPUFREQ_GOV_START:
		...
		snprintf(speedchange_task_name, TASK_NAME_LEN, "speedchange-%s",
					policy->cpu >= NR_CA7 ? "eagle" : "kfc");
		tunables->speedchange_task =
			kthread_create(cpufreq_interactive_speedchange_task, NULL,
				       speedchange_task_name);
		if (IS_ERR(tunables->speedchange_task)) {
			pr_debug("%s: event = %s, policy->cpu = %d end 3\n", __func__, event_str[event], policy->cpu);
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->policy = policy;
			pcpu->floor_freq = policy->cur;
			pcpu->target_freq = get_cpufreq_limit_freq(policy->cpu);
	...
	case CPUFREQ_GOV_STOP:
		...
		if (tunables->speedchange_task && !IS_ERR(tunables->speedchange_task)) {
			kthread_stop(tunables->speedchange_task);
			put_task_struct(tunables->speedchange_task);
			tunables->speedchange_task = NULL;
		} else {
			WARN(1, KERN_ERR "tunables->speedchange_task is invalid!");
		}
	...
	pr_debug("%s: event = %s, policy->cpu = %d end 4\n", __func__, event_str[event], policy->cpu);
	return 0;
}
...
unsigned int cpufreq_interactive_get_down_sample_time(void) {
	struct cpufreq_interactive_cpuinfo *pcpu =
			&per_cpu(cpuinfo, 0);
	struct cpufreq_interactive_tunables *tunables;
#ifdef CONFIG_DOWN_DELAY
	...
	freq_down_delay = freq_to_down_delay(tunables, pcpu->policy->cur);

	return freq_down_delay;
#else
	return tunables->min_sample_time;
#endif
}
#endif
