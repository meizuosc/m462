...
#ifndef MEIZU_SPECIFIC
static DEFINE_PER_CPU(struct cpu_load_info, cur_cpu_info);
#endif
static DEFINE_MUTEX(dm_hotplug_lock);
static DEFINE_MUTEX(dm_thread_lock);
static DEFINE_MUTEX(big_hotplug_lock);
static DEFINE_MUTEX(little_hotplug_in_lock);

static int kfc_num_min_limit = 1;
static int kfc_num_max_limit = NR_CA7;
static int cpu_num_min_limit = 0;
static int cpu_num_max_limit = NR_CA15;

static struct pm_qos_request cpu_num_min_qos;
static struct pm_qos_request cpu_num_max_qos;
static struct pm_qos_request kfc_num_min_qos;
static struct pm_qos_request kfc_num_max_qos;

static int __ref scale_cpu_num(int target_num, int cluster);
static int get_online_cpu_num(int cluster);
static void start_pm_qos_request_unlocked(void);
					...
					const char *buf, size_t count)
{
	...
	if (enable_input) {
		...
		if (big_hotpluged)
#ifdef CONFIG_DEFERRABLE_DM_HOTPLUG
			if(!dynamic_hotplug(CMD_BIG_OUT))
				prev_cmd = CMD_BIG_OUT;
#else
...
// check and control hotplug
static ssize_t show_cpu_hotplug_nums(struct kobject *kobj,
			struct attribute *attr, char *buf){
	int bign = 0 ,littlen = 0;

	mutex_lock(&dm_hotplug_lock);

	bign = get_online_cpu_num(CA15);
	littlen = get_online_cpu_num(CA7);
...
static ssize_t __ref store_cpu_hotplug_nums(struct kobject *kobj, struct attribute *attr,
					const char *buf, size_t count){
	int bign = 0, littlen=0;

	if(!sscanf(buf, "%d %d", &bign, &littlen)){
		printk("invalid command [big number, little number]\n");
		return -EINVAL;
	}

	if((NR_CA7 < littlen) || (NR_CA15 < bign)){
		printk("%s, invalid cpu nums\n", __func__);
		return -EINVAL;
	}

	printk("%s, starting bootst big: %d, little: %d\n",
		__func__, bign, littlen);

	pm_qos_update_request(&cpu_num_min_qos, bign);
	pm_qos_update_request(&kfc_num_min_qos, littlen);
...
static struct global_attr cpu_hotplug_nums =
	__ATTR(cpu_hotplug_nums, S_IRUGO | S_IWUSR,
	show_cpu_hotplug_nums,
	store_cpu_hotplug_nums);
#endif
...
/*
 *	ret value
 *	1 : Disabled and Failed in Hotplug
 * 	0 : Succeed in Hotplug
 *	- : Fail in Hotplug
 */
	...
	if (out_flag) {
		...
		if (cmd == CMD_BIG_OUT && !in_low_power_mode) {
			...
			if (cmd == CMD_LITTLE_ONE_OUT) {
				...
				if (scale_cpu_num(hotplug_out_limit + 1, CA7))
					goto blk_out;
			}
		}
	} else {
		...
		if (cmd == CMD_BIG_IN) {
			...
			if (scale_cpu_num(NR_CA15, CA15))
				goto blk_out;
		} else {
			if (cmd == CMD_LITTLE_ONE_IN) {
				if (scale_cpu_num(NR_CA7 - 1, CA7))
					goto blk_out;
			} else if ((big_hotpluged && !do_disable_hotplug) ||
				(cmd == CMD_LITTLE_IN)) {
				if (scale_cpu_num(NR_CA7, CA7))
					goto blk_out;
			} else {
				if (lcd_is_on) {
					...
					for (i = 1; i < setup_max_cpus; i++) {
						...
						if (i < NR_CA7 && i >= kfc_num_max_limit) {
							i = NR_CA7;
							continue;
						}
...
static int dynamic_hotplug(enum hotplug_cmd cmd)
{
	...
	case CMD_NORMAL:
		ret = __cpu_hotplug(false, cmd);
#ifdef CONFIG_DEFERRABLE_DM_HOTPLUG
		if(lcd_is_on) in_low_power_mode = false;
#else
...
#ifdef CONFIG_DEFERRABLE_DM_HOTPLUG
		if(!dynamic_hotplug(CMD_LITTLE_ONE_IN)) {
			little_hotplug_in++;
			prev_cmd = CMD_LITTLE_ONE_IN;
		}
#else
...
static void event_hotplug_in_work(struct work_struct *work)
{
	int ret;
	mutex_lock(&dm_thread_lock);
	ret = dynamic_hotplug(CMD_NORMAL);
	if(!ret)
		prev_cmd = CMD_NORMAL;
	else if (ret < 0)
...
#ifdef MEIZU_SPECIFIC
	if (!lcd_is_on) nr_running_threshold = DEFAULT_NR_RUN_THRESHD*4;
	else nr_running_threshold = DEFAULT_NR_RUN_THRESHD;
#endif
...
static int on_run(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (!lcd_is_on) {
			// Lower power mode
			if (num_online_cpus() == 1 && !in_suspend_prepared) delay = 2000;
			else delay = 100;
		} else {
			delay = delay_backup;
		}
...
static int __ref scale_cpu_num(int target_num, int cluster)
{
	int cur_num, val, i, ret = 0;

	if (cluster == CA7) {
		cur_num = get_online_cpu_num(CA7);
		target_num = max(target_num, kfc_num_min_limit);
		target_num = min(target_num, kfc_num_max_limit);

		if (target_num > cur_num) {
			val = target_num - cur_num;
			for (i = 1; i < NR_CA7 && val > 0; i++) {
			    if (!cpu_online(i)) {
				ret = cpu_up(i);
				if (ret) {
		...
		cur_num = get_online_cpu_num(CA15);
		target_num = max(target_num, cpu_num_min_limit);
		target_num = min(target_num, cpu_num_max_limit);
		if (target_num > cur_num) {
			val = target_num - cur_num;
			for (i = NR_CA7; i < setup_max_cpus && val > 0; i++) {
			    if (!cpu_online(i)) {
				ret = cpu_up(i);
				if (ret) {
					pr_err("%s: up %d failed\n", __func__, i);
			...
			val = cur_num - target_num;
			for (i = setup_max_cpus - 1; i >= NR_CA7 && val > 0; i--) {
				if (cpu_online(i)) {
					ret = cpu_down(i);
					if (ret) {
						pr_err("%s: down %d failed\n", __func__, i);
...
static void start_pm_qos_request_unlocked(void)
{
	int kfc_online = 0, cpu_online = 0;

	kfc_online = get_online_cpu_num(CA7);
	cpu_online = get_online_cpu_num(CA15);

	pr_info("%s: before: kfc_online %d, cpu_online %d\n", __func__, kfc_online, cpu_online);

	scale_cpu_num(kfc_online, CA7);

	scale_cpu_num(cpu_online, CA15);

	kfc_online = get_online_cpu_num(CA7);
	cpu_online = get_online_cpu_num(CA15);

	pr_info("%s: after: kfc_online %d, cpu_online %d\n", __func__, kfc_online, cpu_online);
}

static int exynos_cpu_num_min_qos_handler(struct notifier_block *b, unsigned long val, void *v)
{
	mutex_lock(&dm_hotplug_lock);

	pr_info("%s: lock minimal cpu num %lu\n", __func__, val);

	cpu_num_min_limit = (int)val;
	start_pm_qos_request_unlocked();
...
static struct notifier_block exynos_cpu_num_min_qos_notifier = {
	.notifier_call = exynos_cpu_num_min_qos_handler,
};

static int exynos_cpu_num_max_qos_handler(struct notifier_block *b, unsigned long val, void *v)
{
	mutex_lock(&dm_hotplug_lock);

	pr_info("%s: lock maximal cpu num %lu\n", __func__, val);

	cpu_num_max_limit = (int)val;
	start_pm_qos_request_unlocked();
	mutex_unlock(&dm_hotplug_lock);
...
static struct notifier_block exynos_cpu_num_max_qos_notifier = {
	.notifier_call = exynos_cpu_num_max_qos_handler,
};

static int exynos_kfc_num_min_qos_handler(struct notifier_block *b, unsigned long val, void *v)
{
	mutex_lock(&dm_hotplug_lock);

	pr_info("%s: lock minimal kfc num %lu\n", __func__, val);

	kfc_num_min_limit = (int)val;
	start_pm_qos_request_unlocked();
...
static struct notifier_block exynos_kfc_num_min_qos_notifier = {
	.notifier_call = exynos_kfc_num_min_qos_handler,
};

static int exynos_kfc_num_max_qos_handler(struct notifier_block *b, unsigned long val, void *v)
{
	mutex_lock(&dm_hotplug_lock);

	pr_info("%s: lock maximal kfc num %lu\n", __func__, val);

	kfc_num_max_limit = (int)val;
	start_pm_qos_request_unlocked();
...
static struct notifier_block exynos_kfc_num_max_qos_notifier = {
	.notifier_call = exynos_kfc_num_max_qos_handler,
};
...
static int __init dm_cpu_hotplug_init(void)
{
	...
	if(sysfs_create_file(power_kobj, &cpu_hotplug_nums.attr)){
		pr_err("%s; fail to create cpu_hotplug_nums sysfs\n",
			__func__);
		goto err_cpu_hotplug_nums;
	}
	...
	pm_qos_add_notifier(PM_QOS_CPU_NUM_MIN, &exynos_cpu_num_min_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CPU_NUM_MAX, &exynos_cpu_num_max_qos_notifier);
	pm_qos_add_notifier(PM_QOS_KFC_NUM_MIN, &exynos_kfc_num_min_qos_notifier);
	pm_qos_add_notifier(PM_QOS_KFC_NUM_MAX, &exynos_kfc_num_max_qos_notifier);

	pm_qos_add_request(&cpu_num_min_qos, PM_QOS_CPU_NUM_MIN, 0);
	pm_qos_add_request(&kfc_num_min_qos, PM_QOS_KFC_NUM_MIN, 0);
	pm_qos_add_request(&cpu_num_max_qos, PM_QOS_CPU_NUM_MAX, NR_CA15);
	pm_qos_add_request(&kfc_num_max_qos, PM_QOS_KFC_NUM_MAX, NR_CA7);
...
err_cpu_hotplug_nums:
	sysfs_remove_file(power_kobj, &cpu_hotplug_nums.attr);
