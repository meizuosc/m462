...
static struct pm_qos_request max_cpu_num_qos;
					...
					const char *buf, size_t count)
{
	...
	if (cpu_input >= (int)freq_min[CA15]) {
		if (egl_hotplugged) {
			pm_qos_update_request(&max_cpu_num_qos, NR_CA15);
		...
		if (cpu_input < 0) {
			...
			if (!egl_hotplugged) {
				pm_qos_update_request(&max_cpu_num_qos, 0);
				egl_hotplugged = true;
			}
		}
	}
...
static int __init exynos_cpufreq_init(void)
{
	...
	pm_qos_add_request(&max_cpu_num_qos, PM_QOS_CPU_NUM_MAX,
					NR_CA15);
