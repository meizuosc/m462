...
struct s5p_ehci_hcd {
	...
	unsigned post_dpm_resume:1;
};

#ifdef CONFIG_NEZHA_HSIC_LINK_PM
extern void set_hsic_active(int value);
#else
static void set_hsic_active(int value) {}
#endif
...
static inline void usb_phy_init_with_pm(struct usb_phy *phy)
{
	usb_phy_init(phy);
...
static inline void usb_phy_shutdown_with_pm(struct usb_phy *phy)
{
...
static void s5p_ehci_phy_init(struct platform_device *pdev)
{
	...
	if (s5p_ehci->phy)
		usb_phy_init_with_pm(s5p_ehci->phy);
				...
				const char *buf, size_t count)
{
	...
	if (!power_on && s5p_ehci->power_on) {
		...
		if (s5p_ehci->phy)
			usb_phy_shutdown_with_pm(s5p_ehci->phy);
...
static int s5p_ehci_probe(struct platform_device *pdev)
{
	 ...
	 * So, wakeup flag init defaults for root hubs.
	 */
	device_set_wakeup_capable(&hcd->self.root_hub->dev, 1);
#if 0
	device_wakeup_enable(&hcd->self.root_hub->dev);
#endif
	...
	s5p_ehci->post_dpm_resume = 0;
	wake_lock_init(&s5p_ehci->pm_lock, WAKE_LOCK_SUSPEND, "s5p-ehci");
	...
	s5p_ehci->power_on = 1;
	/* prevent suspend when EHCI active */
	wake_lock(&s5p_ehci->pm_lock);
...
static int s5p_ehci_remove(struct platform_device *pdev)
{
	...
	s5p_ehci->power_on = 0;
	wake_lock_destroy(&s5p_ehci->pm_lock);
	if (!s5p_ehci->retention)
...
#define CP_PORT		2
#define RETRY_CNT_LIMIT	12

static int s5p_ehci_wait_connect(struct usb_hcd *hcd)
{
	u32 val, retry_cnt = 0;
	...
	do {
		msleep(10);
		val = ehci_readl(ehci,&ehci->regs->port_status[CP_PORT-1]);
	} while (++retry_cnt < RETRY_CNT_LIMIT && !(val & PORT_CONNECT));

	pr_info("%s: retry_cnt = %d\n", __func__, retry_cnt);

	return retry_cnt < RETRY_CNT_LIMIT ? 0 : -ETIMEDOUT;
}

bool s5p_test_and_set_post_dpm_resume(struct usb_device *udev)
{
	struct s5p_ehci_hcd *s5p_ehci;
	struct usb_hcd *hcd = bus_to_hcd(udev->bus);

	if (!(udev->quirks & USB_QUIRK_NO_DPM_RESUME))
		return false;
...
EXPORT_SYMBOL(s5p_test_and_set_post_dpm_resume)
...
static int s5p_ehci_runtime_suspend(struct device *dev)
{
	...
	wake_lock_timeout(&s5p_ehci->pm_lock, msecs_to_jiffies(300));
...
static int s5p_ehci_init_resume(struct device *dev)
{
	struct usb_hcd *hcd = dev_get_drvdata(dev);
	struct s5p_ehci_hcd *s5p_ehci = to_s5p_ehci(hcd);
	...
	// init usb2phy and set active
	pm_runtime_disable(s5p_ehci->phy->dev);
	usb_phy_init_with_pm(s5p_ehci->phy);
...
static int s5p_ehci_runtime_resume(struct device *dev)
{
	...
	if (s5p_ehci->post_dpm_resume)
		return s5p_ehci_init_resume(dev);
...
static int s5p_ehci_suspend(struct device *dev)
{
	...
	set_hsic_active(0);
	if (hcd->self.root_hub->quirks & USB_QUIRK_NO_DPM_RESUME)
		s5p_ehci->post_dpm_resume = 1;
...
static int s5p_ehci_resume(struct device *dev)
{
	...
	dev_dbg(dev, "%s %s\n", s5p_ehci->post_dpm_resume ? "skip" : "enter",
		__func__);
...
static int __init ehci_s5p_init(void)
{
	...
	s5p_ehci_hc_driver.wait_for_cp_resume = s5p_ehci_wait_connect;
