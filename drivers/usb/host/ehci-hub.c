...
static int check_reset_complete (
	...
	if (!(port_status & PORT_PE)) {
		...
		/* HSIC port not enabled after reset, just leave this port */
		if (index == 1) {
			ehci_err(ehci, "hand off error WA(2)\n");
			return port_status;
		}
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_RESET:
			...
			if ((temp & (PORT_PE|PORT_CONNECT)) == PORT_CONNECT
				...
				/*
				 * There is a H/W bug about HSIC port.
				 * In order to reset the HSIC port,
				 * the port have to be disabled before reset.
				 */
				if (wIndex == 1) { /* HSIC port: port 2 */
					temp &= ~PORT_PE;
					ehci_writel(ehci, temp, status_reg);
					mdelay(1);
