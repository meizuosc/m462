...
#include <mach/board-mz-factory-test.h>
...
#include "f_fs.c"
#ifdef CONFIG_NEZHA_GADGET_LDISC
...
static char manufacturer_string[256] = "Android";
static char product_string[256] = "Android";
static char serial_string[256] = "0123456789ABCDEF";
...
{
	strncpy(manufacturer_string, mfg, sizeof(manufacturer_string)-1);
	strncpy(product_string, prod, sizeof(product_string) - 1);
	strncpy(serial_string, sn, sizeof(serial_string) - 1);
...
extern int dwc3_exynos_vbus_event(struct device *dev, int vbus);
...
int marvell_modem_function_bind_config(struct android_usb_function *f,
			struct usb_configuration *c)
{
	return pxa910_acm_bind_config(c, 0);
}
...
int marvell_diag_function_bind_config(struct android_usb_function *f,
					struct usb_configuration *c)
{
	return pxa910_diag_bind_config(c, 1);
}
...
#if defined(CONFIG_FACTORY_KERNEL)
	if (unlikely(mx_is_factory_test_mode(MX_FACTORY_TEST_BT))) {
		strlcpy(buf, "marvell_diag,marvell_modem,adb", sizeof(buf));
		pr_info("%s: %s########\n", __func__, buf);
	} else {
...
#if defined(CONFIG_FACTORY_KERNEL)
		/* rewrite pid/vip at factory test */
		if (unlikely(mx_is_factory_test_mode(MX_FACTORY_TEST_BT))) {
			cdev->desc.idVendor = 0x0BB4 ;
			cdev->desc.idProduct = 0x4E4B;
		}
#endif
...
static int gadget_usb_switch_notify(struct notifier_block *this,
				unsigned long event, void *ptr)
{

	struct android_dev *dev =
		container_of(this, struct android_dev, muic_usb_notifier);
...
extern void check_cable_status(void);
...
static int __init init(void)
{
	...
	wake_lock_init(&dev->wakelock, WAKE_LOCK_SUSPEND, "dwc3-gadget");
	...
	dev->muic_usb_notifier.notifier_call = gadget_usb_switch_notify;
	err = register_muic_notifier(&dev->muic_usb_notifier);
	if (err) {
		usb_composite_unregister(&android_usb_driver);
		pr_err("failed to register muic notifier");
		goto err_create;
	}
...
{
	usb_composite_unregister(&android_usb_driver);
	wake_lock_destroy(&_android_dev->wakelock);
	class_destroy(android_class);
	kfree(_android_dev);
	_android_dev = NULL;
}
