...
#define MTP_CPU_QOS_NUM			(3)
#define MTP_CPU_QOS_FREQ_LOW		(1000000)
#define MTP_CPU_QOS_FREQ_HIGH		(1500000)
#define MTP_MIF_QOS_FREQ_LOW		(413000)
#define MTP_MIF_QOS_FREQ_HIGH		(825000)

static const char mtp_shortname[] = "mtp_usb";

static struct pm_qos_request mtp_cpu_qos;
static struct pm_qos_request mtp_cpu_num_qos;
static struct pm_qos_request mtp_mif_qos;
...
static void send_file_work(struct work_struct *data)
{
	...
	if (dev->xfer_file_length >= 100 * 1024 * 1024) {
		pm_qos_update_request(&mtp_cpu_qos, MTP_CPU_QOS_FREQ_HIGH);
		pm_qos_update_request(&mtp_cpu_num_qos, MTP_CPU_QOS_NUM);
		pm_qos_update_request(&mtp_mif_qos, MTP_MIF_QOS_FREQ_HIGH);
	} else {
		pm_qos_update_request_timeout(&mtp_cpu_qos, MTP_CPU_QOS_FREQ_LOW, 1000000);
		pm_qos_update_request_timeout(&mtp_cpu_num_qos, MTP_CPU_QOS_NUM, 1000000);
		pm_qos_update_request_timeout(&mtp_mif_qos, MTP_MIF_QOS_FREQ_LOW, 1000000);
	}
	...
	pm_qos_update_request(&mtp_cpu_num_qos, 0);
	pm_qos_update_request(&mtp_mif_qos, 0);
...
static int mtp_setup(void)
{
	...
	pm_qos_add_request(&mtp_cpu_qos, PM_QOS_KFC_FREQ_MIN, 0);
	pm_qos_add_request(&mtp_cpu_num_qos, PM_QOS_KFC_NUM_MIN, 0);
	pm_qos_add_request(&mtp_mif_qos, PM_QOS_BUS_THROUGHPUT, 0);
...
err2:
	...
	pm_qos_remove_request(&mtp_cpu_num_qos);
	pm_qos_remove_request(&mtp_mif_qos);
