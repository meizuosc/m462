...
#define ADB_KFC_QOS_FREQ		(1500000)
#define ADB_KFC_QOS_NUM			(3)
#define ADB_MIF_QOS_FREQ		( 543000)
#define ADB_CPU_QOS_FREQ		(1200000)
#define ADB_INT_QOS_FREQ		( 400000)

static struct pm_qos_request adb_cpu_qos;
static struct pm_qos_request adb_kfc_qos;
static struct pm_qos_request adb_kfc_num_qos;
static struct pm_qos_request adb_mif_qos;
static struct pm_qos_request adb_int_qos;
				...
				size_t count, loff_t *pos)
{
	...
	if (adb_pm_qos_enable && count > PM_QOS_REQUEST_SIZE) {
		pm_qos_update_request_timeout(&adb_kfc_qos, ADB_KFC_QOS_FREQ, 2000000);
		pm_qos_update_request_timeout(&adb_kfc_num_qos, ADB_KFC_QOS_NUM, 2000000);
		if (cpu_online(NR_CA7))
			pm_qos_update_request_timeout(&adb_cpu_qos, ADB_CPU_QOS_FREQ, 2000000);
		pm_qos_update_request_timeout(&adb_mif_qos, ADB_MIF_QOS_FREQ, 2000000);
		pm_qos_update_request_timeout(&adb_int_qos, ADB_INT_QOS_FREQ, 2000000);
	}
...
static int adb_setup(void)
{
	...
	pm_qos_add_request(&adb_kfc_qos, PM_QOS_KFC_FREQ_MIN, 0);
	pm_qos_add_request(&adb_kfc_num_qos, PM_QOS_KFC_NUM_MIN, 0);
	pm_qos_add_request(&adb_cpu_qos, PM_QOS_CPU_FREQ_MIN, 0);
	pm_qos_add_request(&adb_mif_qos, PM_QOS_BUS_THROUGHPUT, 0);
	pm_qos_add_request(&adb_int_qos, PM_QOS_DEVICE_THROUGHPUT, 0);
...
static void adb_cleanup(void)
{
	...
	pm_qos_remove_request(&adb_int_qos);
	pm_qos_remove_request(&adb_mif_qos);
	pm_qos_remove_request(&adb_cpu_qos);
	pm_qos_remove_request(&adb_kfc_qos);
	pm_qos_remove_request(&adb_kfc_num_qos);
