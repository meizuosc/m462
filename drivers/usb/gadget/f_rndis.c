...
struct f_rndis {
	...
	struct pm_qos_request rndis_kfc_num_qos;
};
...
/*-------------------------------------------------------------------------*/
static void rndis_cpufreq_lock(void *_rndis)
{
	...
	if (!rndis->cpufreq_lock_status) {
		...
		/* KFC core 2 */
		pm_qos_add_request(&rndis->rndis_kfc_num_qos,
				PM_QOS_KFC_NUM_MIN, 2);
...
static void rndis_cpufreq_unlock(void *_rndis)
{
	struct f_rndis *rndis = _rndis;
	...
	pm_qos_remove_request(&rndis->rndis_kfc_num_qos);
