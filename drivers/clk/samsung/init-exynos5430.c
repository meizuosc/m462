...
void uart_init_clock(void)
{
	...
	exynos_set_rate("dout_sclk_uart0", 200 * 1000000);
	//exynos_set_rate("dout_sclk_uart1", 200 * 1000000);
	//exynos_set_rate("dout_sclk_uart2", 200 * 1000000);
}
...
void clocks_to_oscclk(void)
{
	...
	fout_mphy_pll = clk_get(NULL, "fout_mphy_pll");
	if (IS_ERR(fout_mphy_pll)) {
		pr_err("failed get fout_mphy_pll clock\n");
		goto out_2;
	}

	clk_prepare(fout_mphy_pll);
	clk_enable(fout_mphy_pll);
	clk_disable(fout_mphy_pll);
	clk_unprepare(fout_mphy_pll);
	clk_put(fout_mphy_pll);
out_2:
