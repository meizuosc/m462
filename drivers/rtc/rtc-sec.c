...
static int s2m_rtc_stop_alarm_0_1(struct s2m_rtc_info *info, int index)
{
	int ret, i;
	enum s2m_rtc_reg alarm_index = index ? S2M_ALARM1_SEC : S2M_ALARM0_SEC;

	dev_info(info->dev, "%s: stop alarm%d\n", __func__, index);
	...
	for (i = 0; i < NR_RTC_CNT_REGS; i++) {
		ret = sec_rtc_update(info->iodev, alarm_index + i, 0, ALARM_ENABLE_MASK);
		if (ret < 0) {
			dev_err(info->dev, "%s: Stop alarm%d failed!!!!!\n", __func__, index);
			goto out;
		}
	}
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = sec_rtc_bulk_read(info->iodev, S2M_ALARM0_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
		dev_err(info->dev, "%s:%d fail to read alarm0 reg(%d)\n",
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = sec_rtc_bulk_write(info->iodev, S2M_ALARM0_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to write alarm0 reg(%d)\n",
			__func__, ret);
		goto out;
	}
...
static int s2m_rtc_read_boot_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = sec_rtc_bulk_read(info->iodev, S2M_ALARM1_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_set_boot_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2m_rtc_update(info, S2M_RTC_WAKE);
	if (ret < 0)
		goto out;

	/*update alarm1*/
	ret = sec_rtc_bulk_write(info->iodev, S2M_ALARM1_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to write alarm1 reg(%d)\n",
			__func__, ret);
		goto out;
	}
...
#ifdef CONFIG_RTC_BOOT_ALARM
static irqreturn_t s2m_rtc_alarm_irq1(int irq, void *data)
{
	struct s2m_rtc_info *info = data;
...
#ifdef CONFIG_RTC_BOOT_ALARM
		info->irq1 = irq_base + S2MPS13_IRQ_RTCA1;
#endif
		info->use_alarm_workaround = false;
		/* change to work-around that independent on sys_time */
		break;
	case S2MPS11X:
...
#ifdef CONFIG_RTC_BOOT_ALARM
	ret = devm_request_threaded_irq(&pdev->dev, info->irq1, NULL,
			s2m_rtc_alarm_irq1, 0, "rtc-alarm1", info);
	if (ret < 0) {
		dev_err(&pdev->dev, "Failed to request alarm1 IRQ: %d: %d\n",
			info->irq1, ret);
		goto err_rtc_alarm0_irq;
	}
	disable_irq(info->irq1);
	disable_irq(info->irq1);
#endif
...
static void s2m_rtc_shutdown(struct platform_device *pdev)
{
	...
	/*disable alarm0, disable alarm1 if RTC_WAKE disabled*/
	s2m_rtc_stop_alarm_0_1(info, 0);
	if (!(info->update_reg & RTC_WAKE_MASK))
		s2m_rtc_stop_alarm_0_1(info, 1);
}
