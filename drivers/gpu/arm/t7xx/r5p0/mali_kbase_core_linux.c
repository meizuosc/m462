...
#if MALI_UNIT_TEST
struct kbase_exported_test_data shared_kernel_test_data;
...
#define KBASE_MEM_PROFILE_MAX_BUF_SIZE (64 + (24 * 32 + 64) * 10 + 40)
...
mali_error kbasep_kds_allocate_resource_list_data(struct kbase_context *kctx, struct base_external_resource *ext_res, int num_elems, struct kbase_kds_resource_list_data *resources_list)
{
...
mali_bool kbasep_validate_kbase_pointer(union kbase_pointer *p)
{
#ifdef CONFIG_COMPAT
	if (is_compat_task()) {
		if (p->compat_value == 0)
...
mali_error kbase_external_buffer_lock(struct kbase_context *kctx, struct kbase_uk_ext_buff_kds_data *args, u32 args_size)
{
	...
	if (NULL != ext_res_copy) {
		...
		if (0 == copy_from_user(ext_res_copy, ext_res_user, ext_resource_size)) {
			...
			fdata = kmalloc(sizeof(struct kbasep_kds_resource_set_file_data), GFP_KERNEL);
...
#ifdef CONFIG_MALI_NO_MALI
			unsigned long flags;
			struct kbase_model_control_params params = ((struct kbase_uk_model_control_params *)args)->params;
			/*mutex lock */
			spin_lock_irqsave(&kbdev->reg_op_lock, flags);
...
static int kbase_open(struct inode *inode, struct file *filp)
{
...

	{
		...
		element = kzalloc(sizeof(struct kbasep_kctx_list_element), GFP_KERNEL);
		if (element) {
