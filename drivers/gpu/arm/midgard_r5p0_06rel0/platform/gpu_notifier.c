...
static int gpu_tmu_hot_check_and_work(struct kbase_device *kbdev, unsigned long event)
{
	...
	case GPU_THROTTLING1:
		...
		GPU_LOG(DVFS_ERROR, "[G3D] GPU_THROTTLING_90_95\n");
		break;
	case GPU_THROTTLING2:
		lock_clock = GPU_THROTTLING_95_100;
		GPU_LOG(DVFS_ERROR, "[G3D] GPU_THROTTLING_95_100\n");
		break;
	case GPU_THROTTLING3:
		lock_clock = GPU_THROTTLING_100_105;
		GPU_LOG(DVFS_ERROR, "[G3D] GPU_THROTTLING_100_105\n");
		break;
	case GPU_THROTTLING4:
		lock_clock = GPU_THROTTLING_105_110;
		GPU_LOG(DVFS_ERROR, "[G3D] GPU_THROTTLING_105_110\n");
		break;
	case GPU_TRIPPING:
		lock_clock = GPU_TRIPPING_110;
		GPU_LOG(DVFS_ERROR, "[G3D] GPU_THROTTLING_110\n");
		break;
	default:
...
kbase_pm_callback_conf pm_callbacks = {
   ...
   .power_on_post_callback = gpu_power_on_post,
#else
   .power_on_post_callback = NULL,
#endif
#endif /* SOC_NAME */
