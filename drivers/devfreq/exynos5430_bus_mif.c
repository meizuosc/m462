...
#define TRAFFIC_BYTES_WQHD_32BIT_60FPS_1 (2560*1536*4*60)
#define TRAFFIC_BYTES_WQHD_32BIT_58FPS (2560*1536*4*58)
#define TRAFFIC_BYTES_WQXGA_32BIT_60FPS	(2560*1600*4*60)

#define TRAFFIC_BYTES_32BIT_60FPS	(4*60)

#ifdef CONFIG_NONUNIFORM_DELAY
#define FINE_SAMPLE_FREQ	400000
#define NORMAL_SAMPLE_FREQ	150000
#endif
...
void exynos5_update_media_layers(enum devfreq_media_type media_type, unsigned int value)
{
	...
	case TYPE_RESOLUTION:
		...
		case TRAFFIC_BYTES_WQHD_32BIT_60FPS_1:
		case TRAFFIC_BYTES_WQHD_32BIT_58FPS:
...
#ifdef CONFIG_NONUNIFORM_DELAY
	if	(devfreq_nb->df->previous_freq > FINE_SAMPLE_FREQ) *deferred += FINE_SAMPLE_CODE;
	else if(devfreq_nb->df->previous_freq > NORMAL_SAMPLE_FREQ) *deferred += NORMAL_SAMPLE_CODE;
	else *deferred += COARSE_SAMPLE_CODE;
...
static void exynos5_devfreq_init_thermal(void)
{
	...
	INIT_DEFERRABLE_WORK(&devfreq_mif_ch1_work.devfreq_mif_thermal_work,
			exynos5_devfreq_thermal_monitor);
	/*
	 *
	 * No need to implement the hybrid WQ for the monitor of MIF Thermal
	 * Because the Memory Junction Temperature does not exceed the criteria,
	 *   if the only memory operation is heavy.
	 *
	 */
#else
