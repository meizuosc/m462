					...
					unsigned long *freq)
{
	...
	unsigned long pm_qos_min = 0, pm_qos_max = UINT_MAX, cal_qos_max = 0;
	...
	if (data) {
		...
		pm_qos_max = pm_qos_request(data->pm_qos_class + 1);
	...
	if (pm_qos_min) {
#ifdef CONFIG_MIF_EXPEDITION
		if(*freq == 0 && pm_qos_min > df->min_freq) {
			if(usage_rate > (dfe_idlethreshold/2)) pm_qos_min += 10000;
		}
#endif
