...
static irqreturn_t tsu6721_muic_func(int irq, void *dev)
{
	...
	if(chip->attach){
		if(dev1 & (1<<2)){
			...
			chip->cable_type = CABLE_TYPE_OTG;
		}else{
...
static  int tsu6721_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = request_threaded_irq(chip->irq,tsu6721_muic_isr,tsu6721_muic_func,IRQF_TRIGGER_FALLING,"tsu6721-muic",chip);
...
MODULE_DESCRIPTION("TI TSU6721 muic switch driver for MEIZU m76");
