...
#define FW_IMAGE_NAME "m76-tp.img"
...
#define LCD_TYPE_JDI	0
#define LCD_TYPE_SHARP	1
...
static ssize_t fwu_sysfs_product_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t fwu_sysfs_fwupdate_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static struct image_info m7x_images[] = {   //JDI

{ .product_id = "76TPKW",.img_path = "76TPKW.img", 1622268,-1},
{ .product_id = "76TPKB",.img_path = "76TPKB.img", 1622268,-1},
{ .product_id = "76WTKB",.img_path = "76WTKB.img", 1734582,-1},
{ .product_id = "76WTKW",.img_path = "76WTKW.img", 1734582,-1},
{ .product_id = "76BIEB",.img_path = "76BIEB.img", 1734582,-1},
{ .product_id = "73TPKW",.img_path = "73TPKW1.img", 1622268,-1},
{ .product_id = "73TPKB",.img_path = "73TPKB1.img", 1622268,-1},
{ .product_id = "73BIEB",.img_path = "73BIEB1.img", 1734582,-1},

/*just for test */
{ .product_id = "6WTKB",.img_path = "76WTKB.img", 1734582,-1},
{ .product_id = "TPK-TOL2W",.img_path = "TPK-TOL2W.img", 1622268,-1},
{ .product_id = "WTK-OGSB",.img_path = "WTK-OGSB.img", 1622268,-1},

};

static struct image_info m7x_images_s[] = {  //sharp

{ .product_id = "76TPKWS",.img_path = "76TPKWS.img", 1622268,-1},
{ .product_id = "76TPKBS",.img_path = "76TPKBS.img", 1622268,-1},
{ .product_id = "76WTKBS",.img_path = "76WTKBS.img", 1622268,-1},
{ .product_id = "76WTKWS",.img_path = "76WTKWS.img", 1622268,-1},
{ .product_id = "76BIEBS",.img_path = "76BIEBS.img", 1622268,-1},
{ .product_id = "76TPKOBS",.img_path = "76TPKOBS.img", 1622268,-1},
{ .product_id = "73BIEBS",.img_path = "73BIEBS1.img", 1622268,-1},
{ .product_id = "73TPKOBS",.img_path = "73TPKOBS1.img", 1622268,-1},

/* just for test */
{ .product_id = "76TPKB",.img_path = "76TPKBS.img", 1622268,-1},
{ .product_id = "76TPKB",.img_path = "76TPKB2.img", 1622268,-1},
{ .product_id = "TPK-TOL2W",.img_path = "TPK-TOL2W.img", 1622268,-1},
{ .product_id = "WTK-OGSB",.img_path = "WTK-OGSB.img", 1622268,-1},
{ .product_id = "76TPKW",.img_path = "76TPKWS.img", 1622268,-1},
...
static struct device_attribute attrs[] = {
	__ATTR(doreflash, (S_IRUGO | S_IWGRP|S_IWUSR),
			synaptics_rmi4_show_error,
			fwu_sysfs_do_reflash_store),
	__ATTR(writeconfig, (S_IRUGO | S_IWGRP|S_IWUSR),
			synaptics_rmi4_show_error,
			fwu_sysfs_write_config_store),
	__ATTR(readconfig, (S_IRUGO | S_IWGRP|S_IWUSR),
			synaptics_rmi4_show_error,
			fwu_sysfs_read_config_store),
	__ATTR(configarea, (S_IRUGO | S_IWGRP|S_IWUSR),
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagename, (S_IRUGO | S_IWGRP|S_IWUSR),
			synaptics_rmi4_show_error,
			fwu_sysfs_image_name_store),
	__ATTR(imagesize, (S_IRUGO | S_IWGRP|S_IWUSR),
			synaptics_rmi4_show_error,
	...
	__ATTR(dispconfigblockcount, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(appid, S_IRUGO,
			fwu_sysfs_config_id_show,
			synaptics_rmi4_store_error),
	__ATTR(productid, S_IRUGO,
			fwu_sysfs_product_id_show,
			synaptics_rmi4_store_error),
	__ATTR(fwupdate, S_IRUGO,
			fwu_sysfs_fwupdate_show,
			synaptics_rmi4_store_error),
};
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
...
#else
		image_fw_id = m7x_images[fwu->fw_id].firmware_id;
		fwu->fw_id = image_fw_id;
#endif
	}
	dev_info(rmi4_data->pdev->dev.parent,
	...
	//config_id[4] = 0;
	//strcpy(fwu->config_id, config_id);
	//device_config_id = simple_strtoul(config_id, NULL, 10);
	dev_info(rmi4_data->pdev->dev.parent,
	...
	//image_config_id = simple_strtoul(fwu->config_data, NULL, 10);
	dev_info(rmi4_data->pdev->dev.parent,
	...
	printk("%s: image_config = %d , device_config = %d\n", __func__, image_config_id, device_config_id);
...
static int fwu_start_reflash(void)
{
	...
	pr_notice("%s: product id %s, lcd type is 0x%x\n", __func__,fwu->product_id,meizu_lcd_type);

	//pr_notice("sharp lcd is %d\n", (meizu_lcd_type >> 16) & 1 );
	...
	if (fwu->force_update) {
		...
		if(((meizu_lcd_type >> 16) & 1 ) == LCD_TYPE_JDI)
		{
			for(i = 0; i < (sizeof(m7x_images)/sizeof(m7x_images[0])); i++)
			{
				if(strcmp("s3528", fwu->product_id) == 0) {
					strncpy(fwu->image_name, "76WTKB1.img", MAX_IMAGE_NAME_LEN);
					dev_dbg(rmi4_data->pdev->dev.parent,
							"%s: Requesting firmware image %s\n",
							__func__, fwu->image_name);
				}
				if(strcmp(m7x_images[i].product_id, fwu->product_id) == 0) {
					if((meizu_lcd_type & 0xff) == 1) {
						snprintf(firmware_name, sizeof(firmware_name), "%s%d.img",
						fwu->product_id,(meizu_lcd_type & 0xff));
						...
						strncpy(fwu->image_name, m7x_images[i].img_path, MAX_IMAGE_NAME_LEN);
						dev_dbg(rmi4_data->pdev->dev.parent,
							"%s: Requesting firmware image %s\n",
							__func__, fwu->image_name);
					}
					...
					fwu->fw_id = i;
					break;
				}/*76xxxx.img   76xxxxd.img */
			}
		}else if(((meizu_lcd_type >> 16) & 1) == LCD_TYPE_SHARP){
			for(i = 0; i < (sizeof(m7x_images_s)/sizeof(m7x_images_s[0])); i++)
			{
				if(strcmp(m7x_images_s[i].product_id, fwu->product_id) == 0) {
					if((meizu_lcd_type & 0xff) == 1) {
						snprintf(firmware_name, sizeof(firmware_name), "%s%d.img",
							fwu->product_id,(meizu_lcd_type & 0xff));
					}else{
						snprintf(firmware_name, sizeof(firmware_name), "%sS%d.img",
							fwu->product_id,(meizu_lcd_type & 0xff));
					}
				strncpy(fwu->image_name, firmware_name, MAX_IMAGE_NAME_LEN);
				dev_dbg(rmi4_data->pdev->dev.parent,
	...
	{
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: Firmware image is not found!\n",
			__func__);
		retval = -EINVAL;
		goto exit;
	}
	dev_err(rmi4_data->pdev->dev.parent,
				"%s: Firmware image %s  request begin! \n",
				__func__, fwu->image_name);
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s: Firmware image %s  available  OK! \n",
				__func__, fwu->image_name);
#else
...
#if 0
void  rmi_show_appid(void)
{
	
	unsigned char config_id[5] = {0};
	...
	printk("%s:%s:",__func__,fwu->product_id);
	printk("%s\n",config_id); 
	return ;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	unsigned char config_id[5] = {0};
	char *ic_id = "S3528";
	char *change_product = "76WTKJB";
	char *app_id = ",,";
	int count = 0 ;
	/* device config id */
	retval = synaptics_rmi4_reg_read(fwu->rmi4_data,
				fwu->f34_fd.ctrl_base_addr,
				config_id,
				sizeof(config_id)-1);
	if (retval < 0) {
	...
	dev_dbg(dev,"f34_fd.ctrl_base_addr 0x%X(0x%.2X%.2X%.2X%.2X).\n", fwu->f34_fd.ctrl_base_addr,
		config_id[0],config_id[1],config_id[2],config_id[3]);
	if(strcmp("s3528", fwu->product_id) == 0) {
		count  =  snprintf(buf, PAGE_SIZE, "%s:",change_product);
	}else{
		count  =  snprintf(buf, PAGE_SIZE, "%s:",fwu->product_id);
	}
	
	count += snprintf(buf+count, PAGE_SIZE, "%s:",config_id); 
	count += snprintf(buf+count, PAGE_SIZE, "%s:",ic_id); 
	count += snprintf(buf+count, PAGE_SIZE, "%d",fwu->rmi4_data->firmware_id); 
