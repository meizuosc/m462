...
static ssize_t synaptics_rmi4_mode_control_read(struct device *dev,
		 struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_margin_control_write(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static ssize_t synaptics_rmi4_margin_control_read(struct device *dev,
		 struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_water_control_write(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static ssize_t synaptics_rmi4_water_control_read(struct device *dev,
		 struct device_attribute *attr, char *buf);
...
//				synaptics_rmi4_hall_mode_store),
	...
	__ATTR(margin_control, (S_IRUGO|S_IWUSR|S_IWGRP),
		synaptics_rmi4_margin_control_read,
		synaptics_rmi4_margin_control_write),
	__ATTR(water_control, (S_IRUGO|S_IWUSR|S_IWGRP),
		synaptics_rmi4_water_control_read,
		synaptics_rmi4_water_control_write),
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
 ...
 ssize_t synaptics_rmi4_margin_control_write(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	int retval = -1 ;
...
#endif
		synaptics_rmi4_reg_write(rmi4_data,SYN_MARGIN_CONTROL_ADDR,&enable, 1);

	switch (data[1]) {
		case MAR_LEFT:
			retval = synaptics_rmi4_reg_write(rmi4_data,SYN_LEFT_MARGIN_ADDR,data, 1);
			printk("set SYN_LEFT_MARGIN_ADDR to %d\n", data[0]);
			break;
		case MAR_RIGHT:
			retval = synaptics_rmi4_reg_write(rmi4_data,SYN_RIGHT_MARGIN_ADDR,data, 1);
			printk("set SYN_RIGHT_MARGIN_ADDR to %d\n", data[0]);
			break;
		case MAR_TOP:
			retval = synaptics_rmi4_reg_write(rmi4_data,SYN_TOP_MARGIN_ADDR,data, 1);
			printk("set SYN_TOP_MARGIN_ADDR to %d\n", data[0]);
			break;
		case MAR_BOTTOM:
			retval = synaptics_rmi4_reg_write(rmi4_data,SYN_BOTTOM_MARGIN_ADDR,data, 1);
			printk("set SYN_BOTTOM_MARGIN_ADDR to %d\n", data[0]);
			break;
		default:
			printk("[%s]: write margin mode data[%x] is invalid\n",__func__,data[1]);
	}

	retval = synaptics_rmi4_reg_read(rmi4_data,gesture_only_addr,&enabledata[0],sizeof(enabledata));
	retval |= synaptics_rmi4_reg_write(rmi4_data,gesture_only_addr,&enabledata[0],sizeof(enabledata));
	if(retval<0)
		printk("%s: write margin control mode error \n", __func__);
	 return count ;
}


ssize_t synaptics_rmi4_margin_control_read(struct device *dev,
		 struct device_attribute *attr, char *buf)
 {
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	unsigned char margin_data[4];
	char enable;
	int retval = -1 ;
	int *p = (int *)margin_data;
	retval = synaptics_rmi4_reg_read(rmi4_data,SYN_LEFT_MARGIN_ADDR,margin_data,1);
	retval |= synaptics_rmi4_reg_read(rmi4_data,SYN_RIGHT_MARGIN_ADDR,(margin_data + 1),1);
	retval |= synaptics_rmi4_reg_read(rmi4_data,SYN_TOP_MARGIN_ADDR,(margin_data + 2),1);
	retval |= synaptics_rmi4_reg_read(rmi4_data,SYN_BOTTOM_MARGIN_ADDR,(margin_data + 3),1);
	retval |= synaptics_rmi4_reg_read(rmi4_data,SYN_MARGIN_CONTROL_ADDR,&enable,1);
	memcpy(buf,margin_data,4);
	printk("ENABLE:[%d], left:[%d], right:[%d], top:[%d], bottom:[%d] margin data:[%x]\n",enable,margin_data[0], margin_data[1],
		margin_data[2], margin_data[3], *p);
	return 4 ;
 }

 ssize_t synaptics_rmi4_water_control_write(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	int value ;
	sscanf(buf,"%d",&value);
	rmi4_data->water_control = value;
	synaptics_rmi4_reg_write(rmi4_data,SYN_WATER_CONTROL_ADDR,&rmi4_data->water_control, 1);
	printk("[%s]: water control write %d\n", __func__, value);

	return count;

}

ssize_t synaptics_rmi4_water_control_read(struct device *dev,
		 struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", rmi4_data->water_control);
}

static int synaptics_usb_notity_func(struct notifier_block *nb,
			unsigned long action, void *data)
{
	struct synaptics_rmi4_data * rmi4_data =
				container_of(nb, struct synaptics_rmi4_data,usb_notifier);
	char disable;
	printk("[%s]:usb_notify action[%ld]\n",__func__,action);

	if(USB_PORT_ATTACH == action){
		disable = 1;
		synaptics_rmi4_reg_write(rmi4_data,SYN_WATER_CONTROL_ADDR,&disable, 1);
	}else if(USB_PORT_DETACH == action) {
		disable = 0;
		synaptics_rmi4_reg_write(rmi4_data,SYN_WATER_CONTROL_ADDR,&disable, 1);
...
static void synaptics_register_usb_notify(struct synaptics_rmi4_data *rmi4_data)
{
	rmi4_data->usb_notifier.notifier_call = synaptics_usb_notity_func ;
	register_muic_notifier(&(rmi4_data->usb_notifier));
}
...
#endif
#ifdef WATER_CONTROL_SUPPORT
		synaptics_register_usb_notify(rmi4_data);
#endif
