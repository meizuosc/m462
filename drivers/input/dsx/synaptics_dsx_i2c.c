...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
    ...
    if (IS_ERR(pinctrl))
         pr_err("failed to get TP irq pinctrl - ON");

	bdata->irq_gpio = of_get_named_gpio_flags(np,
			"synaptics,irq-gpio", 0, NULL);

	printk("Get reset-pin, irq:%d\n", bdata->irq_gpio);
	...
	retval = of_property_read_string(np, "synaptics,pwr-reg-name1", &name);
	if (retval == -EINVAL)
		bdata->pwr_reg_name1 = NULL;
	else if (retval < 0)
		return retval;
	else
		bdata->pwr_reg_name1 = name;
