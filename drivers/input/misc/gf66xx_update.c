...
extern int gf66xx_spi_write_bytes(struct gf66xx_dev *gf66xx_dev,u16 addr, u32 data_len, u8 *tx_buf);
extern int gf66xx_spi_read_bytes(struct gf66xx_dev *gf66xx_dev,u16 addr, u32 data_len, u8 *rx_buf);
extern int  gf66xx_hw_reset1(struct gf66xx_dev *gf66xx_dev);

#ifdef CONFIG_INPUT_GX_FP_TEE
extern int gf66xx_spi_write_byte(struct gf66xx_dev *gf66xx_dev, u16 addr, u8 value);
extern int gf66xx_spi_read_byte(struct gf66xx_dev *gf66xx_dev, u16 addr, u8 *value);
#endif

#ifdef CONFIG_INPUT_GX_FP_TEE
#define GF66XX_MAX_SPI_BUF_LEN  (512) //need be same as defined in secure driver
#endif
...
static int write_reg(struct gf66xx_dev* gf66xx_dev, u16 addr, u8 value)
{
	...
	status = gf66xx_spi_write_byte(gf66xx_dev, addr, value);
...
static int read_reg(struct gf66xx_dev* gf66xx_dev, u16 addr, u8* value)
{
	...
	int status = 0;
	//mutex_lock(&gf66xx_dev->buf_lock);
	status = gf66xx_spi_read_byte(gf66xx_dev, addr, value);
	//mutex_unlock(&gf66xx_dev->buf_lock);
	return status;
#else
	int status = 0;
...
{
#ifdef CONFIG_INPUT_GX_FP_TEE
	u32 cnt = 4096 / GF66XX_MAX_SPI_BUF_LEN + 1;
	u32 reminder = 4096 % GF66XX_MAX_SPI_BUF_LEN;
	u32 t_len = 0;

	for (; cnt > 0; cnt--) {
		if ((cnt == 1) && (reminder == 0)){
			return;
		}else if((cnt == 1) && (reminder > 0)){
			t_len = reminder;
		}else{
			t_len = GF66XX_MAX_SPI_BUF_LEN;
		}
		gf66xx_spi_read_bytes(gf66xx_dev, addr, t_len, buf);
...
static void write_4k_bytes(struct gf66xx_dev* gf66xx_dev, u16 addr, u8* buf)
{
	...
	for (; cnt > 0; cnt--) {
		...
		gf66xx_spi_write_bytes(gf66xx_dev, addr, t_len, buf);
...
static int is_init_complete(struct gf66xx_dev* gf66xx_dev)
{
    ...
    while(reg_value != 0xAA)
    {
        ...
        if (time_out > 100)
        {
            printk("[gf66xx_fw_update] timeout = %d\n", time_out);
            return 1;
        }
...
static int download_dsp_isp_9p(struct gf66xx_dev* gf66xx_dev, unsigned char *buf, u16 len)
{
   ...
   // read_data(0xC000, len, tmpBuf);
	...
	printk("[%s]tmpBuf:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n", __func__,
    ...
    {
		printk("[%s]download_dsp_isp_9p 111 Data read back is not the same as the write one.\n", __func__);
		printk("[%s]tmpBuf:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n", __func__,
		tmpbuf[0],tmpbuf[1],tmpbuf[2],tmpbuf[3],tmpbuf[4],tmpbuf[5],tmpbuf[6],tmpbuf[7]);
		printk("[%s]buf:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n", __func__,
...
static int update_patch_block0(struct gf66xx_dev* gf66xx_dev, unsigned char *buf, unsigned short len)
{
    ...
    {
		printk("[gf66xx_fw_update] update_patch_block0 Init is not complete.\n");
        return -1;
    }
#else
	msleep(100);
  ...
  //  gettimeofday(&t1, NULL);
   ...
   // printf("Write use: %ld us\n", 1000000*(t3.tv_sec - t2.tv_sec) + t3.tv_usec - t2.tv_usec);
    ...
    {
	 printk("[gf66xx_fw_update] update_patch_block0 111  Data read back is not the same as the write one.\n");
	 return -1;
    }

    /*7.Send the command(move code[write 0x01 to 0x5094]). Waiting DSP move the code.*/
    write_reg(gf66xx_dev, 0x5094, 0x01);
	printk("[%s]tmpBuf_2:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n", __func__,
		   tmpbuf[0],tmpbuf[1],tmpbuf[2],tmpbuf[3],tmpbuf[4],tmpbuf[5],tmpbuf[6],tmpbuf[7]);
	printk("[%s]-tmpBuf_2:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n", __func__,
			tmpbuf[len-8],tmpbuf[len-7],tmpbuf[len-6],tmpbuf[len-5],tmpbuf[len-4],tmpbuf[len-3],tmpbuf[len-2],tmpbuf[len-1]);
    if (is_burn_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_patch_block0 Code is burnning, not completed.\n");
        return -1;
    }
   ...
   // gettimeofday(&t3, NULL);
    ...
    if (memcmp(buf, tmpbuf, len))
    {
	 printk("[gf66xx_fw_update] update_patch_block0 222  Data read back is not the same as the write one.\n");
	 return -1;
    }
...
static int update_patch_block1(struct gf66xx_dev* gf66xx_dev, unsigned char *buf, unsigned short len)
{
	...
	if (is_init_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_patch_block1 Init is not complete.\n");
        return -1;
    }
#else
	msleep(40);
    ...
    if (memcmp(buf, tmpbuf, len))
    {
	 printk("[gf66xx_fw_update] update_patch_block1 111  Data read back is not the same as the write one.\n");
	 return -1;
    }
    ...
    if (is_burn_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_patch_block1, Code is burnning, not completed.\n");
        return -1;
    }
    ...
    if (memcmp(buf, tmpbuf, len))
    {
		printk("[gf66xx_fw_update] update_patch_block1 222  Data read back is not the same as the write one.\n");
    	return -1;
    }
 ...
 * 5.Hold SS51, release DSP(write 0x4 to 0x4180). Waiting the initialization to complete(0x5094==0xAA)
 * 6.Burn the first 8KB of the firmware to the area begin from 0xC000.
 * 7.Send the command(move code[write 0x03 to 0x5094]). Waiting DSP move the code.
 * 8.Select bank4(write 0x4 to 0x4048)
 * 9.Enable accessing code(write 0x1 to 0x404D)
 * 10.Read 8KB data from 0xC000, check whether burn successfully.
 * 11.Disable accessing code(write 0x00 to 0x404D)
...
static int update_patch_block2(struct gf66xx_dev* gf66xx_dev, unsigned char *buf, unsigned short len)
{
	...
	if (is_init_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_patch_block2 Init is not complete.\n");
        return -1;
    }
#else
	mdelay(100);
  ...
  //  gettimeofday(&t1, NULL);
   ...
   // printf("Write use: %ld us\n", 1000000*(t3.tv_sec - t2.tv_sec) + t3.tv_usec - t2.tv_usec);
    ...
    if (memcmp(buf, tmpbuf, len))
    {
	 printk("[gf66xx_fw_update] update_patch_block2 111  Data read back is not the same as the write one.\n");
	 return -1;
    }
    ...
    if (is_burn_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_patch_block2 Code is burnning, not completed.\n");
        return -1;
    }
   ...
   // gettimeofday(&t3, NULL);
    ...
    if (memcmp(buf, tmpbuf, len))
    {
	 printk("[gf66xx_fw_update] update_patch_block2 222  Data read back is not the same as the write one.\n");
	 return -1;
    }
...
static int update_patch_block3(struct gf66xx_dev* gf66xx_dev, unsigned char *buf, unsigned short len)
{
	...
	if (is_init_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_patch_block3 Init is not complete.\n");
        return -1;
    }
#else
	mdelay(40);
    ...
    if (memcmp(buf, tmpbuf, len))
    {
	 printk("[gf66xx_fw_update] update_patch_block3 111  Data read back is not the same as the write one.\n");
	 return -1;
    }
    ...
    if (is_burn_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_patch_block3 Code is burnning, not completed.\n");
        return -1;
    }
    ...
    if (memcmp(buf, tmpbuf, len))
    {
		printk("[gf66xx_fw_update] update_patch_block3 222  Data read back is not the same as the write one.\n");
		return -1;
    }
...
static int update_dsp_code_9p(struct gf66xx_dev* gf66xx_dev, unsigned char *buf, unsigned short len)
{
	...
	if (is_init_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_dsp_code_9p Init is not complete.\n");
        return -1;
    }
#else
	msleep(40);
    ...
    if (memcmp(buf, tmpbuf, len))
    {
	printk("[%s]update_dsp_code_9p 111 Data read back is not the same as the write one.\n", __func__);
	printk("[%s]tmpBuf:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n", __func__,
		tmpbuf[0],tmpbuf[1],tmpbuf[2],tmpbuf[3],tmpbuf[4],tmpbuf[5],tmpbuf[6],tmpbuf[7]);
	printk("[%s]buf:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n", __func__,
	buf[0],buf[1],buf[2],buf[3],buf[4],buf[5],buf[6],buf[7]);
    ...
    if (is_burn_complete(gf66xx_dev))
    {
		printk("[gf66xx_fw_update] update_dsp_code_9p Code is burnning, not completed.\n");
        return -1;
    }
    ...
    if (memcmp(buf, tmpbuf, len))
    {
		printk("[gf66xx_fw_update] update_dsp_code_9p 222 Data read back is not the same as the write one.\n");
		return -1;
    }
...
int gf66xx_fw_update(struct gf66xx_dev* gf66xx_dev, unsigned char *buf, unsigned short len)
{
	...
	int ret = 0;
	if(buf == NULL) {
		printk("[gf66xx_fw_update] Firmware buffer is NULL.\n");
		return -2;
	}
   ...
   // update_init_9p(gf66xx_dev);/*first initilize the update interface*/
	...
	if (ret == 0)
    {
		...
		if(ret != 0) {
			printk("[gf66xx_fw_update] Failed to download_dsp_isp_9p.int = %d\n", ret);
			return ret;
		} else {
			printk("[gf66xx_fw_update] Download DSP ISP.Len = %d  Succeed! \n", len);
		}
    } else {
	printk("[gf66xx_fw_update] Failed to update_init_9p. int = %d\n", ret);
    }
	...
	if(ret == 0) {
		printk("[gf66xx_fw_update] Update patch code part0.Len = %d  Succeed!\n", len);
	} else {
		printk("[gf66xx_fw_update] Failed to update patch code part0. int = %d\n", ret);
		return ret;
	}
	...
	if(ret == 0) {
		printk("[gf66xx_fw_update] Update patch code part1.Len = %d  Succeed!\n", len);
	} else {
		printk("[gf66xx_fw_update] Failed to update patch code part1. int = %d\n", ret);
		return ret;
	}

	/*Step3:update patch code part2. 8KB[16~24K]*/
    p_fw = buf + (8192 * 2) + 14;
	len = 8192;
	ret = update_patch_block2(gf66xx_dev, p_fw, len);
	if(ret == 0) {
		printk("[gf66xx_fw_update] Update patch code part2.Len = %d  Succeed!\n", len);
	} else {
		printk("[gf66xx_fw_update] Failed to update patch code part2. int = %d\n", ret);
		return ret;
	}

	/*Step4:update patch code part3. 8KB[24~32K]*/
	p_fw = buf + (8192 * 3) + 14;
	len = 8192;
	ret = update_patch_block3(gf66xx_dev, p_fw, len);
	if(ret == 0) {
		printk("[gf66xx_fw_update] Update patch code part3.Len = %d  Succeed!\n", len);
	} else {
		printk("[gf66xx_fw_update] Failed to update patch code part3. int = %d\n", ret);
		return ret;
	}

	/*Step5:update dsp code. 4KB[32K~36K]*/
	p_fw = buf + 32*1024 + 14;
	len = 4096;
	ret = update_dsp_code_9p(gf66xx_dev, p_fw, len);
	if(ret == 0) {
		printk("[gf66xx_fw_update] Update DSP CODE.Len = %d  Succeed!\n", len);
	} else {
		printk("[gf66xx_fw_update] Failed to DSP CODE.int = %d\n", ret);
		return ret;
	}

	printk("[gf66xx_fw_update] fw update succeed============\n");
	return ret;
