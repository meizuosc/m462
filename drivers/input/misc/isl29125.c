...
static s32 XYZCCM_RangeW[3][3] ={
	{	1322L,	10425L,	-2793L},// X col
	{	-1421L,	12231L,	-1772L },// Y col
	{	-5115L, 972L, 21845L}, // Z col
};

static s32 XYZCCM_RangeB[3][3] ={
	{	3800L,	9136L,	-4116L},// X col
	{	1154L,	10755L,	-2770L },// Y col
	{	-7901L, 2153L, 21845L}, // Z col
};
...
void autorange(unsigned long long green)
{
	...
	switch (adc_resolution) {
		case 12:
			...
			break;
		case 16:
			//printk("%s: the optical_range is %d\n", __func__, optical_range);
			switch(optical_range) {
				case 375:
...
static int isl_flush_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}

static int isl_flush_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int sensors_id = 0;
...
static DEVICE_ATTR(flush, ISL29125_SYSFS_PERMISSIONS ,isl_flush_show, isl_flush_set);
...
static void isl29125_work_handler(struct work_struct *work)
{
	...
	/*printk("R=%u, G=%u, B=%u, GIR=%lu, CCT=%d, LUX=%llu, cofe %ld\n",
...
static int isl_sensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(ret < 0){
		printk("%s: isl29125 sensor device create input device failed.\n", __FUNCTION__);
		goto err_id;
	}
	...
	if(meizu_sysfslink_register(&client->dev, LINK_KOBJ_NAME) < 0)
		printk("als create meizu sys link failed!\n");
#ifdef CONFIG_HAS_EARLYSUSPEND
