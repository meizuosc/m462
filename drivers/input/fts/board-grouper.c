...
{
	static bool already_on;

	if (onoff == FTS_POWER_ON) {
		if (already_on)
			printk(KERN_INFO "[FTS] power (3.0v) already on\n");
		else {
			tegra_gpio_enable(TEGRA_GPIO_PH3);
			tegra_gpio_enable(TEGRA_GPIO_PH4);
			tegra_gpio_enable(TEGRA_GPIO_PH6);

			gpio_request(TEGRA_GPIO_PH3, "fts-pwn");
			gpio_direction_output(TEGRA_GPIO_PH3, 1);

			gpio_request(TEGRA_GPIO_PH4, "fts-irq");
			gpio_direction_input(TEGRA_GPIO_PH4);

			gpio_request(TEGRA_GPIO_PH6, "fts-reset");
			gpio_direction_output(TEGRA_GPIO_PH6, 0);
			msleep(20);
			gpio_set_value(TEGRA_GPIO_PH6, 1);
			msleep(100);
			...
			printk(KERN_INFO "[FTS] turn on power (3.0v)\n");
		}
	} else
		printk(KERN_INFO "[FTS] powered off\n");

	return 0;
}

struct fts_i2c_platform_data fts_ts_data = {
	.power = fts_vdd_on,
};

static struct i2c_board_info fts_touch_boardinfo[] = {
	{
		I2C_BOARD_INFO("fts", 0x49),
		.irq = (INT_GPIO_BASE + TEGRA_GPIO_PH4),
		.platform_data = &fts_ts_data,
	}
};
...
{
	fts_vdd_on(1);
	printk(KERN_INFO "[FTS] Touch driver register\n");
	i2c_register_board_info(1, fts_touch_boardinfo, 1);
	return 0;
}
#endif /* CONFIG_TOUCHSCREEN_FTS */
