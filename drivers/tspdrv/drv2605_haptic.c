...
{
	if(gchip){
		mutex_lock(&gchip->haptic_mutex);
		drv2605_haptic_on(gchip,true);
		mutex_unlock(&gchip->haptic_mutex);
	}
}
...
static int drv2605_auto_calib(struct haptic_data *chip)
{
	int ret = 0;
	...
	emmc_partition_rw("bat_model",1,2*512,&motor_calib,sizeof(struct motor_calib));
out:
	return ret;
}
		...
		const char *buf, size_t size)
{
	...
	if(calib){
		...
		emmc_partition_rw("bat_model",0,2*512,&gmotor_calib,sizeof(struct motor_calib));
