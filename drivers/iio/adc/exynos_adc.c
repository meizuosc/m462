...
static struct iio_map exynos_adc_default_iio_maps[] = {
	{
		/* EAR_ADC_3.5 */
		.consumer_dev_name = "adc5",
		.consumer_channel = "adc5",
		.adc_channel_label = "adc5",
	},
	{
		.consumer_dev_name = NULL,
		.consumer_channel = NULL,
		.adc_channel_label = NULL,
	},
};
static int adc_iio_map_register(struct iio_dev *indio_dev,
			struct platform_device *pdev)
{
	int ret;
	struct iio_map *map = exynos_adc_default_iio_maps;

	ret = iio_map_array_register(indio_dev, map);
	if(ret) {
		dev_err(&indio_dev->dev, "iio_map_array_register() failed :  %d\n", ret);
		return ret;
	}

	map->consumer_data = map;
...
static int exynos_adc_probe(struct platform_device *pdev)
{
	...
	adc_iio_map_register(indio_dev, pdev);
	return 0;
