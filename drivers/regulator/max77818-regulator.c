	...
	*max77818_regulator_parse_dt(struct device *dev)
{
	...
	struct device_node *regulators_np, *np=NULL;
	...
	for_each_child_of_node(regulators_np, np) {
		for (i = 0; i < ARRAY_SIZE(max77818_safeout_desc); i++)
			if (!of_node_cmp(np->name, max77818_safeout_desc[i].name))
				break;
		...
		rdata->id = i;
		rdata->initdata = of_get_regulator_init_data(dev, np);
		rdata->of_node = np;
		rdata++;
	}
	of_node_put(np);
...
static int max77818_regulator_probe(struct platform_device *pdev)
{
	...
	max77818->uvlo_irq = regmap_irq_get_virq(iodev->irqc_sys, SYS_IRQ_UVLO);
	max77818->ovlo_irq = regmap_irq_get_virq(iodev->irqc_sys, SYS_IRQ_OVLO);
	max77818->tshdn_irq = regmap_irq_get_virq(iodev->irqc_sys, SYS_IRQ_TSHDN);
#if 0
	max77818->tm_irq = regmap_irq_get_virq(iodev->irqc_sys, SYS_IRQ_TM);
#endif
#if 0
	ret = request_threaded_irq(max77818->uvlo_irq, NULL, max77818_charger_chgin_isr,
			IRQF_TRIGGER_FALLING, "sys_uvlo", max77818);
	if (ret) {
		pr_err("%s: Failed to Reqeust CHGIN IRQ\n", __func__);
		goto err_sys_uvlo;
	}

	ret = request_threaded_irq(max77818->ovlo_irq, NULL, max77818_charger_wcin_isr,
			IRQF_TRIGGER_FALLING, "sys_ovlo", max77818);
	if (ret) {
		pr_err("%s: Failed to Reqeust WCIN IRQ\n", __func__);
		goto err_sys_ovlo;
	}

	ret = request_threaded_irq(max77818->tshdn_irq, NULL, max77818_charger_aicl_isr,
			IRQF_TRIGGER_FALLING, "sys_tshdn", max77818);
	if (ret) {
...
#endif
#if 0
	ret = request_threaded_irq(max77818->tm_irq, NULL,	max77818_charger_byp_isr,
			IRQF_TRIGGER_FALLING, "sys_tm", max77818);
	if (ret) {
		pr_err("%s: Failed to Reqeust BYP IRQ\n", __func__);
		goto err_sys_tm;
	}
#endif
	return 0;
